...
use streamfy_protocol::link::versions::{ApiVersions, ApiVersionsRequest, ApiVersionsResponse};
use streamfy_future::net::{DomainConnector, DefaultDomainConnector};
use streamfy_future::retry::retry_if;

use crate::{SocketError, StreamfySocket, SharedMultiplexerSocket, AsyncResponse};
...
impl VersionedSocket {
    ...
    pub fn split(self) -> (StreamfySocket, Arc<ClientConfig>, Versions) {
        (self.socket, self.config, self.versions)
    }
}

/// Low level configuration option to directly connect to Streamfy
...
impl ClientConfig {
    ...
    pub async fn connect(self) -> Result<VersionedSocket, SocketError> {
        ...
        let socket =
            StreamfySocket::connect_with_connector(&self.addr, self.connector.as_ref()).await?;
...
mod test {
    ...
    use streamfy_protocol::api::Request;
    use streamfy_protocol::link::versions::ApiVersionKey;
