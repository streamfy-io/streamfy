...
use streamfy_future::net::{BoxReadConnection};
use streamfy_protocol::api::{ApiMessage, Request, RequestMessage, ResponseMessage};
...
type FrameStream = FramedRead<Compat<BoxReadConnection>, StreamfyCodec>;

/// inner flv stream which is generic over stream
pub struct StreamfyStream {
    inner: FrameStream,
    id: ConnectionFd,
}

impl Debug for StreamfyStream {
...
impl StreamfyStream {
    pub fn new(id: ConnectionFd, stream: BoxReadConnection) -> Self {
        Self {
            inner: FramedRead::new(stream.compat(), StreamfyCodec::new()),
            id,
        }
    }
    ...
    where
        RequestMessage<R>: StreamfyDecoder + Debug,
    {
...
/*
impl From<FrameStream> for StreamfyStream {
    fn from(stream: FrameStream) -> Self {
        StreamfyStream {stream)
    }
}
*/
