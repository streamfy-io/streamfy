...
use streamfy_protocol::codec::StreamfyCodec;
use streamfy_protocol::Encoder as FlvEncoder;
use streamfy_protocol::Version;
use streamfy_future::net::{BoxWriteConnection, ConnectionFd};

use crate::SocketError;

type SinkFrame = FramedWrite<Compat<BoxWriteConnection>, StreamfyCodec>;

pub struct StreamfySink {
    inner: SinkFrame,
    fd: ConnectionFd,
    enable_zero_copy: bool,
}

impl fmt::Debug for StreamfySink {
...
impl StreamfySink {
    ...
    pub fn new(sink: BoxWriteConnection, fd: ConnectionFd) -> Self {
        Self {
            ...
            inner: SinkFrame::new(sink.compat_write(), StreamfyCodec::new()),
        }
    }
...
mod fd {
    ...
    use super::StreamfySink;

    impl AsRawFd for StreamfySink {
...
mod file {
    ...
    use bytes::BytesMut;
    use streamfy_future::task::spawn_blocking;
    use futures_util::AsyncWriteExt;
    use nix::sys::uio::pread;

    use streamfy_protocol::store::{FileWrite, StoreValue};
    use streamfy_future::zero_copy::ZeroCopy;
    ...
    mod tests {
        ...
        use streamfy_future::net::TcpListener;
        use streamfy_protocol::Version;
        use streamfy_protocol::store::FileWrite;
        use streamfy_protocol::store::StoreValue;
        use streamfy_future::fs::util;
        use streamfy_future::fs::AsyncFileExtension;
        use streamfy_future::timer::sleep;
        use streamfy_protocol::{Decoder, Encoder};
        use streamfy_types::event::StickyEvent;

        use crate::StreamfySocket;
        use crate::SocketError;
        ...
        async fn test_server(
            ...
            let mut socket: StreamfySocket = incoming_stream.into();
        ...
        async fn setup_client(addr: &str, end: Arc<StickyEvent>) -> Result<(), SocketError> {
            ...
            let mut socket = StreamfySocket::connect(addr).await.expect("connect");
...
pub struct ExclusiveFlvSink {
    inner: Arc<Mutex<StreamfySink>>,
    fd: ConnectionFd,
}

impl ExclusiveFlvSink {
    pub fn new(sink: StreamfySink) -> Self {
        let fd = sink.id();
        ExclusiveFlvSink {
...
impl ExclusiveFlvSink {
    pub async fn lock(&self) -> MutexGuard<'_, StreamfySink> {
