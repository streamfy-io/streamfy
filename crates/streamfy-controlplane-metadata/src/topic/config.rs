...
use streamfy_types::{ReplicationFactor, TopicName, PartitionCount, IgnoreRackAssignment};
...
deduplication:
  ...
  filter:
    transform:
      uses: streamfy/dedup-bloom-filter@0.1.0
"#;
...
"#
    ...
    fn test_deduplication() -> Deduplication {
        Deduplication {
            ...
            filter: Filter {
                transform: Transform {
                    uses: "streamfy/dedup-bloom-filter@0.1.0".to_string(),
