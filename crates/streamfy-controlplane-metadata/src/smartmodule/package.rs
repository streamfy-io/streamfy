...
use streamfy_protocol::{Encoder, Decoder, Version};
...
pub struct SmartModulePackage {
    pub name: String,
    pub group: String,
    pub version: StreamfySemVersion,
    pub api_version: StreamfySemVersion,
    pub description: Option<String>,
    pub license: Option<String>,

    #[streamfy(min_version = 19)]
    #[cfg_attr(
        feature = "use_serde",
...
pub struct SmartModulePackageKey {
    ...
    pub version: Option<StreamfySemVersion>,
}
    ...
    pub fn from_qualified_name(fqdn: &str) -> Result<Self, SmartModuleKeyError> {
        ...
        if let Some(name_part) = split.next() {
            ...
            if let Some(version_part) = version_split.next() {
                ...
                pkg.version = Some(StreamfySemVersion::new(
...
#[cfg_attr(feature = "use_serde", derive(serde::Serialize, serde::Deserialize))]
pub struct StreamfySemVersion(SemVersion);

impl StreamfySemVersion {
...
impl Default for StreamfySemVersion {
    fn default() -> Self {
        Self(SemVersion::new(0, 1, 0))
    }
}

impl Display for StreamfySemVersion {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}", self.0)
    }
}

impl Encoder for StreamfySemVersion {
    fn write_size(&self, version: streamfy_protocol::Version) -> usize {
    ...
    fn encode<T>(
        &self,
        dest: &mut T,
        version: streamfy_protocol::Version,
    ) -> Result<(), std::io::Error>
    where
        T: bytes::BufMut,
    {
...
impl Decoder for StreamfySemVersion {
...
mod package_test {
    ...
    use super::{SmartModulePackage, StreamfySemVersion};
    ...
    fn test_pkg_fqdn() {
        let pkg = SmartModulePackage {
            name: "test".to_owned(),
            group: "streamfy".to_owned(),
            version: StreamfySemVersion::parse("0.1.0").unwrap(),
            api_version: StreamfySemVersion::parse("0.1.0").unwrap(),
            ..Default::default()
        };

        assert_eq!(pkg.fqdn(), "streamfy/test@0.1.0");
    }
    ...
    fn test_pkg_name() {
        ...
        assert_eq!(pkg.store_id(), "test-streamfy-0.1.0");
    }
    ...
    fn test_pkg_key_fully_qualified() {
        ...
        assert_eq!(
            pkg.version,
            Some(StreamfySemVersion::parse("0.1.0").expect("parse"))
        );
    }
...
mod test {
    ...
    use super::{StreamfySemVersion, SmartModulePackage};
    ...
    fn write_metadata_toml() {
        ...
        let param = SmartModuleParam {
            optional: true,
            description: Some("streamfy".to_owned()),
        };
    ...
    fn read_metadata_toml() {
        ...
        assert_eq!(
            ...
            StreamfySemVersion::parse("0.1.0").unwrap()
        );
        ...
        assert_eq!(
            ...
            "https://github.com/streamfy/streamfy"
        );
