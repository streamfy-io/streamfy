        ...
        match self {
            ...
            _ => {
                match new {
                    ...
                    Value::Bool(_val) => Ok(Diff::Replace(new.clone())), // for now, we only support replace
                    Value::Number(_val) => Ok(Diff::Replace(new.clone())),
                    Value::String(_val) => Ok(Diff::Replace(new.clone())),
                    Value::Array(_val) => Ok(Diff::Replace(new.clone())),
                    Value::Object(new_val) => match self {
                        Value::Object(old_val) => {
