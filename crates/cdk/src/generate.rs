...
enum CdkTemplateValue {
    ...
    Group(String),
    Description(String),
    ConnStreamfyDependencyHash(String),
...
impl Display for CdkTemplateValue {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            CdkTemplateValue::ConnStreamfyDependencyHash(hash) => {
                write!(f, "streamfy-cargo-dependency-hash={hash}")
            }
...
impl CdkTemplateUserValues {
    fn new() -> Self {
        // By default the streamfy dependency hash is the current git hash
        // and its always passed as option to cargo generate
        let values = vec![CdkTemplateValue::ConnStreamfyDependencyHash(
