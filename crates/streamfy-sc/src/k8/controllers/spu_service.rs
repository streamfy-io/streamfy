...
impl SpuServiceController {
    ...
    async fn inner_loop(&mut self) -> Result<()> {
        ...
        if let Some(config) = self.configs.store().value("streamfy").await {
    ...
    async fn sync_with_spg(&mut self, listener: &mut K8ChangeListener<SpuGroupSpec>) -> Result<()> {
        ...
        if let Some(config) = self.configs.store().value("streamfy").await {
            ...
            return Err(anyhow!("streamfy config not found"));
        }
    ...
    async fn spply_spu_load_service(
        ...
        use streamfy_stream_model::k8_types::core::service::ServiceSpec as K8ServiceSpec;

        let mut selector = HashMap::new();
        let pod_name = format!("streamfy-spg-{spu_name}");
        ...
        let mut ctx = spg_obj
            .ctx()
            .create_child()
            .set_labels(vec![("streamfy.io/spu-name", spu_name)]);
        ctx.item_mut()
            .annotations
