...
use anyhow::Result;
use streamfy_stream_dispatcher::store::K8ChangeListener;
use streamfy_stream_model::store::k8::K8MetaItem;
use tracing::debug;
use tracing::error;
use tracing::trace;
use tracing::warn;
...
impl SpgStatefulSetController {
    ...
    async fn sync_spgs_to_statefulset(
        ...
        if let Some(config_obj) = self.configs.store().value("streamfy").await {
            ...
            error!("streamfy config map not found, skipping");
        }
...
mod test {
    ...
    use streamfy_stream_dispatcher::actions::WSAction;
    use streamfy_stream_dispatcher::dispatcher::MetadataDispatcher;
