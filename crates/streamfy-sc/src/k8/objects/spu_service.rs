...
use streamfy_stream_model::k8_types::core::service::LoadBalancerIngress;
...
impl SpuServiceSpec {
    ...
    pub fn service_name(spu_name: &str) -> String {
        format!("streamfy-spu-{spu_name}")
    }

    pub fn spu_name(meta: &ObjectMeta) -> Option<&String> {
        meta.labels.get("streamfy.io/spu-name")
    }

    pub fn ingress_annotation(meta: &ObjectMeta) -> Option<&String> {
        meta.annotations.get("streamfy.io/ingress-address")
    }
}
...
mod extended {
    ...
    use streamfy_stream_model::k8_types::core::service::ServiceSpec;
