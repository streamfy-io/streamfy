...
impl SpuGroupObj {
    pub fn new(inner: MetadataStoreObject<SpuGroupSpec, K8MetaItem>) -> Self {
        let svc_name = format!("streamfy-spg-{}", inner.key());
        Self { inner, svc_name }
    }
    ...
    pub fn as_statefulset(
        ...
        let statefulset_name = format!("streamfy-spg-{}", self.key());
    ...
    pub fn as_spu(
        ...
        let private_svc_fqdn = format!(
            "streamfy-spg-main-{spu}.streamfy-spg-{}.{ns}.svc.cluster.local",
            self.key()
        );
        let public_svc_fqdn = format!("streamfy-spu-{spu_name}.{ns}.svc.cluster.local");
        ...
        let mut ctx = spg_obj.ctx().create_child().set_labels(vec![(
            "streamfy.io/spu-group".to_string(),
...
mod k8_convert {
    ...
    use streamfy_stream_model::k8_types::*;
    ...
    use streamfy_stream_model::k8_types::app::stateful::{
    ...
    pub fn generate_k8_stateful(
        ...
        let mut args = vec![
            "/streamfy-run".to_owned(),
            "spu".to_owned(),
            "--sc-addr".to_owned(),
            format!("streamfy-sc-internal.{namespace}.svc.cluster.local:{SC_PRIVATE_PORT}"),
            ...
            if tls.enable_client_cert {
                ...
                volumes.push(VolumeSpec {
                    ...
                    secret: Some(SecretVolumeSpec {
                        secret_name: "streamfy-ca".to_owned(), // fixed
                        ..Default::default()
                    }),
                    ..Default::default()
                });
    ...
    fn test_as_spu_id_private_endpoint_for_k8s() {
        ...
        for spu in spu_cases {
            ...
            assert_eq!(
                ...
                format!("streamfy-spg-main-{spu}.streamfy-spg-{spu}.default.svc.cluster.local")
            );
