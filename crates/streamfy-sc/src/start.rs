...
use streamfy_future::{task::run_block_on, timer::sleep};
use streamfy_stream_dispatcher::metadata::{SharedClient, MetadataClient, local::LocalMetadataStorage};
use streamfy_stream_model::{store::k8::K8MetaItem, core::MetadataItem};
...
pub fn main_loop(opt: ScOpt) {
    ...
    match opt.mode() {
        ...
        RunMode::ReadOnly(read_only_path) => {
            ...
            let client = streamfy_future::task::run_block_on(async move {
...
mod proxy {
    ...
    use streamfy_types::print_cli_err;
...
async fn create_memory_client(path: PathBuf) -> Result<Arc<MemoryClient>> {
    use std::ops::Deref;
    use streamfy_sc_schema::remote_file::RemoteMetadataFile;
