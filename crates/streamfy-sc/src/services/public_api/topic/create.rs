...
//!
//! Computed Topics use Streamfy algorithm for replica assignment.
...
use streamfy_sc_schema::objects::CreateRequest;
    ...
    if let Some(deduplication) = topic_spec.get_deduplication() {
        ...
        if !metadata.smartmodules().store().contains_key(&sm_fqdn).await {
            return Status::new(
                ...
                Some(format!(
                    "{}\nHint: try `streamfy hub sm download {sm_name}` and repeat this operation",
                    ErrorCode::DeduplicationSmartModuleNotLoaded
                )),
            );
        }
