...
use streamfy::{
    dataplane::record::RecordData, DeliverySemantic, Streamfy, Isolation,
...
pub(crate) struct ProducerWorker {
    streamfy_producer: TopicProducerPool,
...
impl ProducerWorker {
    pub(crate) async fn new(
        ...
        let streamfy_config = TopicProducerConfigBuilder::default()
            .callback(callback)
        ...
        let streamfy_producer = streamfy
            .topic_producer_with_config(config.topic_name.clone(), streamfy_config)
            .await?;
    ...
    pub async fn send_batch(self) -> Result<()> {
        ...
        self.streamfy_producer.flush().await?;
