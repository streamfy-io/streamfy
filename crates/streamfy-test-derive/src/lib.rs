...
use streamfy_test_util::test_meta::derive_attr::TestRequirements;

/// This macro will allow a test writer to override
/// minimum Streamfy cluster requirements for a test
///
/// Supported keys:
/// * `min_spu` (default `1`)
...
/// * `async` (default: false)
///
/// #[streamfy_test(min_spu = 2)]
/// pub async fn run(client: Arc<Streamfy>, mut test_case: TestCase) {
///     println!("Test")
/// }
///
/// Will expand into the following pseudocode
///
/// pub async fn run(client: Streamfy, option: TestCase) {
...
pub fn streamfy_test(args: TokenStream, input: TokenStream) -> TokenStream {
    ...
    let output_fn = quote! {
        ...
        pub fn #validate_sub_fn_iden(subcmd: Vec<String>) -> Box<dyn streamfy_test_util::test_meta::TestOption> {
            Box::new(#test_opt_ident::parse_from(subcmd))
        }

        pub fn #requirements_fn_iden() -> streamfy_test_util::test_meta::derive_attr::TestRequirements {
            serde_json::from_str(#fn_test_reqs_str).expect("Could not deserialize test reqs")
        }

        inventory::submit!{
            streamfy_test_util::test_runner::test_meta::StreamfyTestMeta {
                name: #test_name,
        ...
        pub fn #user_test_fn_iden(mut test_driver: streamfy_test_util::test_runner::test_driver::TestDriver, test_case: streamfy_test_util::test_meta::TestCase) {
            use streamfy_test_util::test_meta::environment::EnvDetail;
            #maybe_async_test
        }

        pub fn #test_driver_iden(mut test_driver: streamfy_test_util::test_runner::test_driver::TestDriver, mut test_case: streamfy_test_util::test_meta::TestCase) -> Result<streamfy_test_util::test_meta::test_result::TestResult, streamfy_test_util::test_meta::test_result::TestResult> {
            use streamfy_test_util::test_meta::test_result::TestResult;
            use streamfy_test_util::test_meta::environment::EnvDetail;
            use streamfy_test_util::test_meta::derive_attr::TestRequirements;
            use streamfy_test_util::test_meta::test_timer::TestTimer;
            use streamfy_test_util::test_runner::test_driver::TestDriver;
            use streamfy_test_util::test_runner::test_meta::StreamfyTestMeta;
            use streamfy_test_util::{async_process,fork_and_wait};
            ...
            if TestDriver::is_env_acceptable(&test_reqs, &test_case) {
                ...
                StreamfyTestMeta::customize_test(&test_reqs, &mut test_case);
