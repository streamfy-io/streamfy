...
pub use streamfy_connector_package::render_config_str;
pub use streamfy_connector_package::secret;

#[cfg(feature = "derive")]
pub use streamfy_connector_derive::connector;

use streamfy::{Offset, metadata::topic::TopicSpec};
...
pub mod future {
    pub use streamfy_future::task::run_block_on;
    pub use tokio::select;
    pub use streamfy_future::subscriber::init_logger;
    pub use streamfy_future::retry;
}
...
pub async fn ensure_topic_exists(config: &config::ConnectorConfig) -> Result<()> {
    ...
    let admin = streamfy::StreamfyAdmin::connect().await?;
