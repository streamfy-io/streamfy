...
use streamfy_future::net::unix::UnixListener;
use tracing::{error, info, trace};
use serde::Serialize;
use streamfy_smartengine::metrics::SmartModuleChainMetrics;

const SOCKET_PATH: &str = "/tmp/streamfy-connector.sock";

#[derive(Debug, Serialize)]
pub struct ConnectorMetrics {
    #[serde(flatten)]
    streamfy_metrics: Arc<ClientMetrics>,
...
impl Default for ConnectorMetrics {
    fn default() -> Self {
        Self {
            streamfy_metrics: Arc::new(ClientMetrics::new()),
...
impl ConnectorMetrics {
    pub fn new(streamfy_metrics: Arc<ClientMetrics>) -> Self {
        Self {
            streamfy_metrics,
            smartmodule_metrics: HashMap::new(),
        }
...
async fn start_monitoring(metrics: Arc<ConnectorMetrics>) -> Result<(), IoError> {
    let metric_out_path = match std::env::var("STREAMFY_METRIC_CONNECTOR") {
        Ok(path) => {
    ...
    loop {
        ...
        streamfy_future::timer::sleep(std::time::Duration::from_secs(5)).await;
    }
}
