use streamfy::{StreamfyClusterConfig, SmartModuleInvocation, SmartModuleKind, SmartModuleExtraParams};

use crate::{config::ConnectorConfig, Result};

pub async fn smartmodule_chain_from_config(
    config: &ConnectorConfig,
) -> Result<Option<streamfy::SmartModuleChainBuilder>> {
    ...
    for step in transforms {
        ...
        let config = streamfy::SmartModuleConfig::from(step.clone());
    ...
    Some(
        transforms
            ...
            .map(|s| SmartModuleInvocation {
                wasm: streamfy::SmartModuleInvocationWasm::Predefined(s.uses.clone()),
...
mod tests {
    ...
    use streamfy::SmartModuleInvocationWasm;
    use streamfy_connector_package::config::ConnectorConfigV1;
