...
use streamfy::consumer::{BoxConsumerStream, ConsumerConfigExtBuilder, OffsetManagementStrategy};
use streamfy::{Streamfy, StreamfyClusterConfig, Offset};
use streamfy_connector_package::config::{ConsumerPartitionConfig, OffsetConfig, OffsetStrategyConfig};
use crate::{config::ConnectorConfig, Result};
use crate::ensure_topic_exists;
use crate::smartmodule::smartmodule_vec_from_config;

pub use streamfy::consumer::ConsumerStream;
...
pub async fn consumer_stream_from_config(
    ...
    builder.offset_start(streamfy::Offset::end());
    ...
    let stream = streamfy.consumer_with_config(cfg).await?;

    Ok((streamfy, Box::pin(stream)))
}
