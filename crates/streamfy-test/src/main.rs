...
use streamfy_test_util::test_meta::{BaseCli, TestCase, TestCli, TestOption};
use streamfy_test_util::test_meta::test_result::TestResult;
use streamfy_test_util::test_meta::environment::{EnvDetail, EnvironmentSetup};
use streamfy_test_util::setup::TestCluster;
use streamfy_test_util::test_runner::test_driver::TestDriver;
use streamfy_test_util::test_runner::test_meta::StreamfyTestMeta;
use streamfy_test_util::async_process;

// This is important for `inventory` crate
#[allow(unused_imports)]
use streamfy_test::tests as _;
use sysinfo::{System, get_current_pid, Signal, Process, Pid};
use tracing::debug;

//const CI_FAIL_FLAG: &str = "/tmp/CI_STREAMFY_TEST_FAIL";
...
fn main() {
    ...
    let test_meta = StreamfyTestMeta::from_name(test_name.clone())
    ...
    println!("Start running streamfy test runner");
    ...
    let cluster_setup_wait = async_process!(
        async {
            ...
            if option.cluster_start() || option.remove_cluster_before() {
                ...
                println!("Testing connection to Streamfy cluster in profile");
                Streamfy::connect()
                    .await
                    .expect("Unable to connect to Streamfy test cluster via profile");
            }
        },
        "cluster setup"
    );
...
mod tests {
    ...
    use clap::Parser;
    use streamfy_test_util::test_meta::{BaseCli, TestCli};
    use streamfy_test_util::test_meta::environment::EnvDetail;
    use streamfy_test::tests::smoke::SmokeTestOption;

    #[test]
    fn valid_test_name() {
        let args = BaseCli::try_parse_from(vec!["streamfy-test", "smoke"]);
    ...
    fn invalid_test_name() {
        let args = BaseCli::try_parse_from(vec!["streamfy-test", "testdoesnotexist"]);
    ...
    #[test]
    fn spu() {
        let args = BaseCli::parse_from(vec!["streamfy-test", "smoke", "--spu", "5"]);
    ...
    #[test]
    fn timeout() {
        let args = BaseCli::parse_from(vec!["streamfy-test", "smoke", "--timeout", "9000s"]);
    ...
    //// 'Failed to install k8 cluster: InstallK8(HelmChartNotFound("streamfy/streamfy-app:a.b.c-xyz"))'
    //// This won't work with k8 until we fix: https://github.com/streamfy/streamfy/issues/859
    ////
    ...
    //        let _streamfy_client = cluster_setup(&test_case).await;
    //        let cluster_addr_1 = {
