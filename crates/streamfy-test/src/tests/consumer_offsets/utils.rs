...
use streamfy::{
    ...
    Streamfy, Offset, RecordKey,
};
use streamfy_controlplane_metadata::topic::TopicSpec;
use streamfy_future::timer::sleep;
use streamfy_protocol::{link::ErrorCode, record::ConsumerRecord};
...
pub(crate) async fn wait_for_offsets_topic_provisined(client: &Streamfy) -> Result<()> {
    for _ in 0..5 {
        ...
        if system_topics
            .iter()
            .any(|t| t.name.eq(streamfy_types::defaults::CONSUMER_STORAGE_TOPIC))
        {
            return Ok(());
        }
