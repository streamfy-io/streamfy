...
use streamfy::{TopicProducerPool, StreamfyAdmin, RecordKey, TopicProducerConfigBuilder};
...
#[command(name = "Streamfy Producer Batch Test")]
pub struct ProduceBatchTestOption {}

#[streamfy_test(topic = "batch", async)]
pub async fn produce_batch(
    mut test_driver: Arc<StreamfyTestDriver>,
    ...
    let leader = {
        let admin: StreamfyAdmin = test_driver.client().admin().await;
    ...
    let result: Result<_> = async move {
        ...
        streamfy_future::timer::sleep(std::time::Duration::from_secs(3)).await;
