...
pub use streamfy_controlplane_metadata::topic::config as topic_config;
pub use streamfy_smartengine::transformation::TransformationStep;
pub use streamfy_types::PartitionId;
pub use streamfy_types::compression::Compression;
...
const IMAGE_PREFFIX: &str = "streamfy/streamfy-connect";
...
mod tests {
    ...
    use streamfy_smartengine::transformation::{TransformationStep, Lookback};
    ...
    fn full_yaml_test() {
        ...
        let expected = ConnectorConfig::V0_1_0(ConnectorConfigV1 {
            ...
            transforms: vec![TransformationStep {
                uses: "streamfy/json-sql".to_string(),
                lookback: None,
                with: BTreeMap::from([
                    (
                        "mapping".to_string(),
            ...
            meta: MetaConfigV2 {
                ...
                topic: topic_config::TopicConfig {
                    ...
                    deduplication: Some(Deduplication {
                        ...
                        filter: Filter {
                            transform: Transform {
                                uses: "streamfy/streamfy-smartmodule-filter-lookback@0.1.0"
                                    .to_string(),
    ...
    fn test_deserialize_transform() {
        ...
        assert_eq!(connector_spec.transforms()[0].uses.as_str(), "streamfy/sql");
        assert_eq!(
...
",
    ...
    fn test_deserialize_transform_many() {
        ...
        assert_eq!(transform[0].uses.as_str(), "streamfy/json-sql");
        assert_eq!(
            transform[0].with,
            BTreeMap::from([(
                "mapping".to_string(),
        ...
        assert_eq!(transform[1].uses.as_str(), "streamfy/avro-sql");
        assert_eq!(transform[1].with, BTreeMap::default());
        assert_eq!(transform[2].uses.as_str(), "streamfy/regex-filter");
        assert_eq!(
            transform[2].with,
