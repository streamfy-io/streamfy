...
use streamfy_sc_schema::smartmodule::SmartModuleSpec;
use tracing::debug;
use clap::Parser;
...
use streamfy::metadata::topic::CompressionAlgorithm;

use streamfy_controlplane_metadata::topic::config::TopicConfig;
use streamfy_sc_schema::shared::validate_resource_name;
use streamfy_sc_schema::mirror::MirrorSpec;
use streamfy_sc_schema::topic::HomeMirrorConfig;
use streamfy_sc_schema::topic::MirrorConfig;
use streamfy_sc_schema::topic::Bounds;
use streamfy_sc_schema::topic::Deduplication;
use streamfy_sc_schema::topic::Filter;
use streamfy_sc_schema::topic::Transform;
...
const DEFAULT_DEDUP_FILTER: &str = "streamfy/dedup-bloom-filter@0.1.0";
...
impl CreateTopicOpt {
    ...
    async fn construct(self, admin: &StreamfyAdmin) -> Result<(String, TopicSpec)> {
        ...
        use streamfy::metadata::topic::{PartitionMaps, TopicReplicaParam};
        ...
        if self.setting.dedup {
            ...
            if sm.is_none() {
                return Err(CliError::InvalidArg(format!(
                    "Deduplication SmartModule filter '{DEFAULT_DEDUP_FILTER}' not found. Please build and add to the cluster the smartmodule from https://github.com/streamfy/dedup-bloom-filter."
                ))
                .into());
            }
...
mod load {
    ...
    use streamfy::metadata::topic::{PartitionMaps, MirrorConfig};
    use streamfy_controlplane_metadata::topic::HomeMirrorConfig;
    ...
    mod test {
        ...
        use streamfy_controlplane_metadata::{
