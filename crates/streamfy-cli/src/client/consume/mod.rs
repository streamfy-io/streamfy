...
mod cmd {
    ...
    use streamfy_protocol::record::NO_TIMESTAMP;
    use streamfy::metadata::tableformat::TableFormatSpec;
    use streamfy::{Streamfy, Offset, StreamfyError};
    use streamfy::consumer::{ConsumerConfigExt, ConsumerStream, OffsetManagementStrategy};

    use streamfy::consumer::Record;
    ...
    use streamfy_smartengine::transformation::TransformationConfig;
    ...
    pub struct ConsumeOpt {
        ...
        /// Eg. streamfy consume topic-name --smartmodule my_filter -e foo=bar -e key=value -e one=1
        #[arg(
            short = 'e',
        ...
        /// E.g. streamfy consume topic-name --transforms-line='{"uses":"streamfy/jolt@0.1.0","with":{"spec":"[{\"operation\":\"default\",\"spec\":{\"source\":\"test\"}}]"}}'
    ...
    impl ClientCmd for ConsumeOpt {
        ...
        async fn process_client<O: Terminal + Debug + Send + Sync>(
            ...
            init_monitoring(streamfy.metrics());

            //println!("client id",streamfy);
            ...
            self.consume_records(streamfy, maybe_tableformat).await?;
    ...
    impl ConsumeOpt {
        ...
        pub async fn consume_records(
            ...
            self.print_status();
            let mut stream = streamfy.consumer_with_config(consume_config).await?;
                        ...
                        stream_next = stream.next().fuse() => match stream_next {
                            Some(result) => {
                                ...
                                let record = match result {
                                    Ok(record) => record,
                                    /*
                                    Err(StreamfyError::AdminApi(ApiError::Code(code, _))) => {
