...
impl ListConsumerOpt {
    pub async fn process<O>(self, out: std::sync::Arc<O>, streamfy: &Streamfy) -> Result<()>
    where
        O: Terminal,
    {
        let consumers = streamfy.consumer_offsets().await?;
...
mod display {
    ...
    use streamfy::consumer::ConsumerOffset;
    use serde::Serialize;
