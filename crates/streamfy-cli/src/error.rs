...
#[cfg(feature = "k8s")]
use streamfy_cluster::cli::ClusterCliError;
...
pub enum CliError {
    ...
    #[cfg(feature = "k8s")]
    #[error("Streamfy cluster error: {0}")]
    ...
    #[error("Streamfy connector config: {0}")]
    ConnectorConfig(#[from] serde_yaml::Error),

    #[error("Package index error: {0}")]
    IndexError(#[from] streamfy_index::Error),
    #[error("Error finding executable: {0}")]
    WhichError(#[from] which::Error),
    #[error("Http Error: {0}")]
    HttpError(#[from] streamfy_cli_common::error::HttpError),
