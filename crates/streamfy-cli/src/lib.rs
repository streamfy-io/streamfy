...
mod util {
    use streamfy_spu_schema::Isolation;
    use crate::CliError;
...
mod root {
    ...
    #[cfg(feature = "k8s")]
    use streamfy_cluster::cli::ClusterCmd;
    use streamfy_cli_common::install::streamfy_extensions_dir;
    use streamfy_channel::STREAMFY_RELEASE_CHANNEL;

    use crate::profile::ProfileOpt;
    use crate::client::StreamfyCmd;
    ...
    #[derive(Debug, Parser)]
    #[command(
        about = "Streamfy Command Line Interface",
        name = "streamfy",
        help_template = COMMAND_TEMPLATE,
        max_term_width = 100,
        disable_version_flag = true,
        // streamfy consume help would interpret help as topic name, so force -h or --help.
        disable_help_subcommand = true,
        // VersionlessSubcommands is now default behaviour. See https://github.com/clap-rs/clap/pull/2831
        // global_setting = AppSettings::DeriveDisplayOrder
        )]
    enum RootCmd {
        /// All top-level commands that require a Streamfy client are bundled in `StreamfyCmd`
        #[command(flatten)]
        ...
        ///
        /// Each Profile describes a particular Streamfy cluster you may be connected to.
        /// This might correspond to Streamfy running on Minikube or in the Cloud.
        /// There is one "active" profile, which determines which cluster all of the
        /// Streamfy CLI commands interact with.
        #[command(name = "profile")]
        Profile(ProfileOpt),

        /// Install or uninstall Streamfy cluster
        ///
        #[cfg(feature = "k8s")]
        #[command(subcommand, name = "cluster")]
        Cluster(Box<ClusterCmd>),

        /// Print Streamfy version information
        #[command(name = "version")]
        Version(VersionOpt),

        /// Generate command-line completions for Streamfy
        ///
        /// Run the following two commands to enable streamfy command completions.
        ///
        /// Open a new terminal for the changes to take effect.
        ///
        /// $ streamfy completions bash > ~/streamfy_completions.sh
        /// {n}$ echo "source ~/streamfy_completions.sh" >> ~/.bashrc
        #[command(subcommand, name = "completions")]
        Completions(CompletionCmd),

        /// Generate metadata for Streamfy base CLI
    ...
    struct CompletionOpt {
        #[arg(long, default_value = "streamfy")]
        name: String,
    }
    ...
    /// Search for a Streamfy plugin in the following places:
    ///
    /// - In the directory where the `streamfy` executable is located
    /// - In the system PATH
    /// - In the `~/.streamfy/extensions/` directory
    fn find_plugin(name: &str) -> Option<PathBuf> {
        let ext_dir = streamfy_extensions_dir().ok();
    ...
    fn process_external_subcommand(mut args: Vec<String>) -> Result<()> {
        ...
        let subcommand = format!("streamfy-{cmd}");
        let subcommand_path = match find_plugin(&subcommand) {
            Some(path) => path,
            None => {
                match streamfy_extensions_dir() {
                    Ok(streamfy_dir) => {
                        println!(
