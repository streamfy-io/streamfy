...
use streamfy::config::{ConfigFile, TlsConfig, TlsPolicy};
...
impl ExportOpt {
    pub fn process<O: Terminal>(self, out: Arc<O>) -> Result<()> {
        ...
        let config_file = match ConfigFile::load(None) {
            ...
            Err(e) => {
                eprintln!("Unable to find Streamfy config file");
                return Err(e.into());
            }
        };
        ...
        let profile_export = if let Some(streamfy_config) =
            config_file.config().cluster(&cluster_name)
        {
            if let TlsPolicy::Verified(TlsConfig::Files(_)) = streamfy_config.tls {
                return Err(anyhow!(
