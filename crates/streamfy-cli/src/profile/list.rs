...
use streamfy::config::{ConfigFile, Config, TlsPolicy};
use streamfy_extension_common::{Terminal, OutputFormat};
use streamfy_extension_common::output::{TableOutputHandler, OutputType};
...
impl ListOpt {
    pub async fn process<O: Terminal>(self, out: Arc<O>) -> Result<()> {
        let config_file = match ConfigFile::load(None) {
            Ok(config) => config,
            Err(_) => {
                eprintln!("Unable to find Streamfy config");
                eprintln!("Try using 'streamfy cloud login' or 'streamfy cluster start'");
                return Ok(());
            }
        };
