...
use streamfy::config::{Profile, ConfigFile};
...
impl K8Opt {
    pub async fn process(self) -> Result<()> {
        let external_addr = match discover_streamfy_addr(self.namespace.as_deref()).await? {
            Some(sc_addr) => sc_addr,
            None => return Err(anyhow!("streamfy service is not deployed")),
        };
...
pub async fn set_k8_context(opt: K8Opt, external_addr: String) -> Result<Profile> {
    ...
    match config.cluster_mut(&profile_name) {
        ...
        None => {
            let mut local_cluster = StreamfyClusterConfig::new(external_addr);
...
/// find streamfy addr
pub async fn discover_streamfy_addr(namespace: Option<&str>) -> Result<Option<String>> {
    let ns = namespace.unwrap_or("default");
    let maybe_svc = K8Client::try_default()?
        .retrieve_item::<ServiceSpec, _>(&InputObjectMeta::named("streamfy-sc-public", ns))
        .await?;
