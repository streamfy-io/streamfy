...
use streamfy_index::{HttpAgent, PackageId, Target, WithVersion, Package, PackageVersion};

use crate::STREAMFY_EXTENSIONS_DIR;
use crate::error::PackageNotFound;

pub const STREAMFY_DIR: &str = "STREAMFY_DIR";

#[cfg(feature = "default")]
use streamfy_types::defaults::CLI_CONFIG_PATH;
#[cfg(not(feature = "default"))]
pub const CLI_CONFIG_PATH: &str = ".streamfy";

pub fn streamfy_base_dir() -> Result<PathBuf> {
    if let Ok(dir) = std::env::var(STREAMFY_DIR) {
        // Assume this is like `~/.streamfy
        let path = PathBuf::from(dir);
        return streamfy_base_dir_create(path);
    }
    let home =
        home::home_dir().ok_or_else(|| IoError::new(ErrorKind::NotFound, "Homedir not found"))?;
    let path = home.join(CLI_CONFIG_PATH);

    streamfy_base_dir_create(path)
}

pub fn streamfy_bin_dir() -> Result<PathBuf> {
    Ok(streamfy_base_dir()?.join("bin"))
}

fn streamfy_base_dir_create(path: PathBuf) -> Result<PathBuf> {
    if !path.exists() {
...
pub fn streamfy_extensions_dir() -> Result<PathBuf> {
    // Check if STREAMFY_EXTENSIONS_DIR exists for extensions location
    if let Ok(dir_path) = std::env::var(STREAMFY_EXTENSIONS_DIR) {
        Ok(dir_path.into())
    } else {
        let base_dir = streamfy_base_dir()?;
...
// Add to count by skipping anything that starts with `streamfy-`
// I'm not sure if this is how things get added to the CLI though
pub fn get_extensions() -> Result<Vec<PathBuf>> {
    use std::fs;
    let mut extensions = Vec::new();
    let streamfy_dir = streamfy_extensions_dir()?;
    if let Ok(entries) = fs::read_dir(streamfy_dir) {
        for entry in entries.flatten() {
            let is_plugin = entry.file_name().to_string_lossy().starts_with("streamfy-");
            if is_plugin {
...
pub async fn fetch_package_file(
    ...
    if !verify_checksum(&package_file, &package_checksum) {
        return Err(streamfy_index::Error::ChecksumError.into());
    }
...
pub fn install_bin<P: AsRef<Path>, B: AsRef<[u8]>>(bin_path: P, bytes: B) -> Result<()> {
    ...
    let tmp_path = tmp_dir.path().join("streamfy-exe-tmp");
...
pub fn install_println<S: AsRef<str>>(string: S) {
    if std::env::var("STREAMFY_BOOTSTRAP").is_ok() {
        println!("\x1B[1;34mstreamfy:\x1B[0m {}", string.as_ref());
    } else {
