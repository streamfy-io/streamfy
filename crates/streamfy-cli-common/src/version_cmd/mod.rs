...
/// A helper struct for printing Streamfy version information in a standardised
/// format across all Streamfy Products.
///
/// # Example
///
/// ```ignore
/// let mut streamfy_version_printer = StreamfyVersionPrinter::new("streamfy", "0.7.0");
///
/// streamfy_version_printer.append_extra("Release Channel", "stable");
/// streamfy_version_printer.append_extra("Git Commit", "abcdefg");
/// streamfy_version_printer.append_extra("OS Details", "Linux 5.4.0-42-generic (kernel 4.19.76-linuxkit)");
/// println!("{}", streamfy_version_printer);
/// ```
///
#[derive(Clone)]
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
pub struct StreamfyVersionPrinter {
    name: String,
    version: String,
...
impl StreamfyVersionPrinter {
    ...
    /// `extra` field in a new copy of this [`StreamfyVersionPrinter`].
    #[cfg(feature = "serde")]
...
impl Display for StreamfyVersionPrinter {
...
#[cfg(test)]
mod test {
    use super::StreamfyVersionPrinter;

    #[test]
    fn version_output_as_table() {
        let mut streamfy_version_printer = StreamfyVersionPrinter::new("Streamfy CLI", "0.7.0");

        streamfy_version_printer.append_extra("Release Channel", "stable");
        streamfy_version_printer.append_extra("Git Commit", "abcdefg");
        streamfy_version_printer.append_extra(
            "OS Details",
            "Linux 5.4.0-42-generic (kernel 4.19.76-linuxkit)",
        );

        let lines = format!("{streamfy_version_printer}")
            .lines()
            .map(String::from)
            .collect::<Vec<String>>();

        assert!(lines[0].contains("Streamfy CLI"));
        assert!(lines[0].contains("0.7.0"));
        assert!(lines[1].contains("Streamfy CLI Arch"));
        assert!(lines[1].contains(&streamfy_version_printer.arch()));
        assert!(lines[2].contains("Streamfy CLI SHA256"));
    ...
    fn creates_json_output() {
        let mut version_printer = StreamfyVersionPrinter::new("Unicorn CLI", "0.11.0");
