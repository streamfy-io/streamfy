...
pub struct BaseTestCmd {
    ...
    /// Eg. streamfy consume topic-name --filter filter.wasm -e foo=bar -e key=value -e one=1
    #[arg(
        short = 'e',
        long= "params",
    ...
    /// E.g. smdk test --text '{}' --transforms-line='{"uses":"streamfy/jolt@0.1.0","with":{"spec":"[{\"operation\":\"default\",\"spec\":{\"source\":\"test\"}}]"}}'
    ...
    chain
        .look_back(|lookback| {
            let n = match lookback {
                streamfy_smartengine::Lookback::Last(n) => n,
                streamfy_smartengine::Lookback::Age { age: _, last } => last,
            };
            let res = Ok(records
                .clone()
                .into_iter()
...
async fn build_chain(
    ...
    let client_config = StreamfyClusterConfig::load()?.try_into()?;
