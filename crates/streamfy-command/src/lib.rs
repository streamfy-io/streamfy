...
pub enum CommandErrorKind {
    ...
    #[error(
        "Child process completed with non-zero exit code {code}\n  stdout: {stdout}\n  stderr: {stderr}"
    )]
    ExitError {
        /// Process exit code
        code: i32,
...
pub trait CommandExt {
    ...
    /// ```
    /// use std::process::{Command, Output};
    /// use streamfy_command::{CommandExt, CommandErrorKind};
    ///
    ...
    ///     .result()
    ///     .unwrap_err();
    /// if let CommandErrorKind::ExitError { code: 1i32, stderr, .. } = error.source {
    ///     assert_eq!(stderr, "this command failed with this stderr\n");
    /// } else {
        ...
        self.output()
            ...
            .and_then(|output| match output.status.code() {
                ...
                Some(code) => Err(CommandError {
                    command: self.display(),
                    source: CommandErrorKind::ExitError {
                        code,
