...
use streamfy_protocol::record::{Record, RecordSet};
...
async fn handle_response(socket: &mut StreamfySocket, replica: &FileReplica) {
...
async fn test_fetch(addr: &str, iteration: i16, offset: i64, expected_batch_len: usize) {
    let mut socket = StreamfySocket::connect(addr)
        .await
