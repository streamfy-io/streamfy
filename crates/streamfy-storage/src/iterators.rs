...
use streamfy_protocol::{Decoder, Version};

use streamfy_protocol::record::{Batch, Offset, BATCH_FILE_HEADER_SIZE, BATCH_HEADER_SIZE, Record};
...
mod test {
    ...
    fn test_file_record_iterator() -> anyhow::Result<()> {
        ...
        let slice = run_block_on(replica.read_partition_slice(
            0,
            u32::MAX,
            streamfy_spu_schema::Isolation::ReadUncommitted,
        ))?;
