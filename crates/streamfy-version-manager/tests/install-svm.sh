...
# A number of short aliases are also supported for use with STREAMFY_ARCH
#
...
get_architecture() {
    ...
    if [ -n "${STREAMFY_ARCH}" ]; then
        RETVAL="${STREAMFY_ARCH}"
        return 0
    fi
...
say() {
    printf '\e[1;34mstreamfy:\e[0m %s\n' "$1"
}

err() {
    printf '\e[1;31mstreamfy:\e[0m %s\n' "$1" >&2
}
...
abort_prompt_issue() {
    ...
    err "    https://github.com/streamfy-io/streamfy/issues/new"
    exit 1
}
