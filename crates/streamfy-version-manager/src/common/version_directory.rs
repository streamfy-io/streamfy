...
impl VersionDirectory {
    ...
    /// Sets this version as the active Streamfy Version
    pub fn set_active(&self) -> Result<()> {
        // Verify `~/.streamfy/bin` exists and create it if it doesn't
        let streamfy_bin_dir = streamfy_binaries_path()?;

        if !streamfy_bin_dir.exists() {
            tracing::info!(?streamfy_bin_dir, "Creating streamfy binaries directory");
            create_dir_all(&streamfy_bin_dir)?;
        }
        ...
        for entry in &self.contents {
            ...
            let target_path = streamfy_bin_dir.join(filename);
...
mod tests {
    ...
    const TEST_BINARY_NAME: &str = "streamfy";
    ...
    fn sets_as_active_directory() {
        ...
        // Create streamfy directory
        let streamfy_bin_dir = streamfy_binaries_path().unwrap();

        if streamfy_bin_dir.exists() {
            remove_dir_all(&streamfy_bin_dir).unwrap();
        }

        create_dir_all(&streamfy_bin_dir).unwrap();
        ...
        let streamfy_path = streamfy_bin_dir.join(TEST_BINARY_NAME);
        let tempdir_path = tmpdir.path().join(TEST_BINARY_NAME);

        assert!(streamfy_path.exists(), "streamfy binary should exist");
        assert!(tempdir_path.exists(), "tempdir binary should exist");
        assert!(
            version_dir.contents.contains(&tempdir_path),
            "version directory must include binary path as content"
    ...
    fn creates_pakageset_instance_from_version_dir() {
        let version_manifest = VersionManifest {
            ...
            contents: Some(vec![
                VersionedArtifact {
                    name: String::from("streamfy"),
                    version: String::from("0.11.8"),
                },
                VersionedArtifact {
                    name: String::from("streamfy-cloud"),
