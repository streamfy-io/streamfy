...
//!
//! The `switch` command is responsible of changing the active Streamfy Version
...
impl SwitchOpt {
    pub async fn process(&self, notify: Notify) -> Result<()> {
        ...
        // Ensure the `~/.svm/versions` directory exists given that we get
        // installed binaries from there. Without this directory we cant
        // switch versions.
        let versions_path = svm_versions_path()?;

        if !versions_path.exists() {
            notify.warn("No local Streamfy versions found.");
            notify.help(format!(
        ...
        // e.g. Version: 0.10.13 -> ~/.svm/versions/0.10.13
        ...
        if !pkgset_path.exists() {
            ...
            let help = format!("svm install {version}");
