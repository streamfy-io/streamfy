...
impl<T> PackageId<T> {
    ...
    ///
    /// ```
    /// # use streamfy_index::{PackageId, MaybeVersion, WithVersion};
    /// let pid1: PackageId<MaybeVersion> = "streamfy/streamfy".parse().unwrap();
    /// let pid2: PackageId<WithVersion> = "https://packages.streamfy.io/v1/streamfy/streamfy:1.2.3".parse().unwrap();
...
mod tests {
    ...
    fn test_deserialize_group() {
        let group: GroupName = "streamfy.io".parse().unwrap();
        ...
        assert!("streamfy/io".parse::<GroupName>().is_err());

        let bad_json_group = JsonGroup {
            group: GroupName("streamfy/io".to_string()),
        };
    ...
    fn test_parse_package_id_default_registry() {
        let package_id: PackageId<WithVersion> = "streamfy.io/streamfy:0.6.0".parse().unwrap();
    ...
    fn test_parse_package_id_default_group() {
        let package_id: PackageId<WithVersion> = "streamfy-cloud:0.1.4".parse().unwrap();
        assert_eq!(package_id.registry(), &Registry::default());
        assert_eq!(package_id.group().as_str(), "streamfy");
        assert_eq!(package_id.name().as_str(), "streamfy-cloud");
        assert_eq!(
            package_id.version(),
    ...
    fn test_parse_package_id_default_group_maybe_version() {
        let package_id: PackageId = "streamfy-cloud".parse().unwrap();
    ...
    fn test_package_id_display() {
        let package_id_with_version = "streamfy/streamfy:1.2.3-alpha"
            .parse::<PackageId<WithVersion>>()
            .unwrap();
        assert_eq!(
            "streamfy/streamfy:1.2.3-alpha",
            format!("{package_id_with_version}")
        );

        let package_id_maybe_with_version: PackageId<MaybeVersion> =
            "streamfy/streamfy:3.4.5-beta".parse().unwrap();
        assert_eq!(
            "streamfy/streamfy:3.4.5-beta",
            format!("{package_id_maybe_with_version}")
        );

        let package_id_maybe_without_version =
            PackageId::new_unversioned("streamfy".parse().unwrap(), "streamfy".parse().unwrap());
        assert_eq!(
            "streamfy/streamfy",
    ...
    fn test_pretty_no_group() {
        let package_id: PackageId = "streamfy:1.2.3".parse().unwrap();
        let pretty = format!("{}", package_id.pretty());
        assert_eq!(pretty, "streamfy");
    }

    #[test]
    fn test_pretty_group() {
        let package_id: PackageId = "streamfy/streamfy:1.2.3".parse().unwrap();
        let pretty = format!("{}", package_id.pretty());
        assert_eq!(pretty, "streamfy");
    }
    ...
    fn test_pretty_maybe_id_no_group() {
        let package_id: PackageId<MaybeVersion> = "streamfy".parse().unwrap();
        let pretty = format!("{}", package_id.pretty());
        assert_eq!(pretty, "streamfy");
    }
    ...
    fn test_pretty_maybe_id_group() {
        let package_id: PackageId<MaybeVersion> = "streamfy/streamfy".parse().unwrap();
        let pretty = format!("{}", package_id.pretty());
        assert_eq!(pretty, "streamfy");
    }
    ...
    fn test_serialize_package_id_unversioned() {
        ...
        assert_eq!(json, r#""streamfy/streamfy""#);
    }

    #[test]
    fn test_serialize_package_id_versioned() {
        let package_id_with_version = "streamfy/streamfy:1.2.3"
            .parse::<PackageId<WithVersion>>()
            .unwrap();
        let json = serde_json::to_string(&package_id_with_version).unwrap();
        assert_eq!(json, r#""streamfy/streamfy:1.2.3""#);
    }
}
