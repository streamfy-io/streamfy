...
use crate::{Result, StreamfyIndex, Package, PackageId, Target, TagName};
...
impl HttpAgent {
    ...
    pub async fn index_from_response(&self, response: &[u8]) -> Result<StreamfyIndex> {
        let index: StreamfyIndex = serde_json::from_slice(response)?;
        Ok(index)
    }
