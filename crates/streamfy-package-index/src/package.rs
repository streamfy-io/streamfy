...
/// A `Package` represents a single published item in Streamfy's registry.
///
...
/// A `Release` is a specific version of a published item in Streamfy's registry.
...
mod tests {
    ...
    fn test_serialize_package() {
        let id: PackageId<MaybeVersion> = "streamfy/streamfy".parse().unwrap();
        let package = Package::new_binary(&id, "Bob", "A package", "https://github.com");
        let stringified = serde_json::to_string(&package).unwrap();
        assert_eq!(
            stringified,
            r#"{"name":"streamfy","group":"streamfy","kind":"bin","author":"Bob","description":"A package","repository":"https://github.com"}"#
        )
    }
    ...
    fn test_deserialize_package() {
        let json = r#"{
          ...
          "group": "streamfy",
          "kind": "bin",
          "author": "Streamfy Contributors",
          "description": "The Streamfy CLI, an all-in-one toolkit for streaming with Streamfy",
          "repository": "https://github.com/streamfy/streamfy",
          "releases": [
            { 
              "version": "0.8.4",
              "yanked": false,
        ...
        assert_eq!(package.name, "streamfy".parse().unwrap());
        assert_eq!(package.group, "streamfy".parse().unwrap());
        assert_eq!(package.kind, PackageKind::Binary);
        assert_eq!(package.author, Some("Streamfy Contributors".to_string()));
        assert_eq!(
            package.description,
            Some("The Streamfy CLI, an all-in-one toolkit for streaming with Streamfy".to_string())
        );
    ...
    fn test_deserialize_package_unknown_kind() {
        let json = r#"{
          "name": "streamfy-smartmodule-filter",
          "group": "streamfy",
          "kind": "wasm",
          "releases": []
        }"#;
        let package: Package = serde_json::from_str(json).unwrap();
        assert_eq!(package.name, "streamfy-smartmodule-filter".parse().unwrap());
        assert_eq!(package.group, "streamfy".parse().unwrap());
