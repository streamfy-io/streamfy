...
use streamfy::metrics::ClientMetrics;
use streamfy::{StreamfyError, PartitionConsumer};
use streamfy::spu::SpuDirectory;
use streamfy_controlplane_metadata::partition::ReplicaKey;
use streamfy_socket::{ClientConfig, MultiplexerSocket, StreamSocket, VersionedSerialSocket};
...
impl LeaderConnections {
    ...
    async fn connect_to_leader(&self, leader: SpuId) -> Result<StreamSocket, StreamfyError> {
        if let Some(spu) = self.spus.spec(&leader) {
            ...
            Err(StreamfyError::SPUNotFound(leader))
        }
    }
...
impl SpuDirectory for LeaderConnections {
    async fn create_serial_socket(
        &self,
        replica: &ReplicaKey,
    ) -> Result<VersionedSerialSocket, streamfy::StreamfyError> {
        if let Some(replica_spec) = self.replicas.spec(replica) {
            ...
            Err(StreamfyError::TopicNotFound(replica.to_string()))
        }
    }

    async fn create_stream_with_version<R: streamfy_protocol::api::Request>(
        &self,
        replica: &ReplicaKey,
        request: R,
        version: i16,
    ) -> Result<streamfy_socket::AsyncResponse<R>, streamfy::StreamfyError>
    where
        R: Sync + Send,
    {
        if let Some(replica_spec) = self.replicas.spec(replica) {
