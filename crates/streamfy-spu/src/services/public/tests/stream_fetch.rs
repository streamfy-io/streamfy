...
use streamfy_controlplane::spu_api::update_smartmodule::SmartModule;
use streamfy_smartmodule::dataplane::smartmodule::Lookback;
...
use streamfy_protocol::{api::RequestMessage, record::RecordSet};
...
async fn test_stream_fetch_basic() {
    ...
    let client_socket =
        MultiplexerSocket::new(StreamfySocket::connect(&addr).await.expect("connect"));
...
const STREAMFY_WASM_FILTER: &str = "streamfy_smartmodule_filter";

#[streamfy_future::test(ignore)]
async fn test_stream_fetch_filter_adhoc() {
    adhoc_test(
        "test_stream_fetch_filter_adhoc",
        STREAMFY_WASM_FILTER,
...
async fn test_stream_fetch_filter_individual(
    ...
    tokio::select! {
        ...
        _ = streamfy_future::timer::sleep(std::time::Duration::from_millis(1000)) => (),
    }
...
const STREAMFY_WASM_MAP_DOUBLE: &str = "streamfy_wasm_map_double";

#[streamfy_future::test(ignore)]
async fn test_stream_fetch_map_adhoc() {
    adhoc_test(
        "test_stream_fetch_map_adhoc",
        STREAMFY_WASM_MAP_DOUBLE,
        SmartModuleKind::Map,
...
async fn test_stream_fetch_map_adhoc_chain() {
    adhoc_chain_test(
        ...
        &[
            (STREAMFY_WASM_MAP_DOUBLE, SmartModuleKind::Map),
            (STREAMFY_WASM_MAP_DOUBLE, SmartModuleKind::Map),
            (STREAMFY_WASM_MAP_DOUBLE, SmartModuleKind::Map),
        ],
...
async fn test_stream_fetch_and_new_request(
    ...
    let client_socket =
        MultiplexerSocket::shared(StreamfySocket::connect(&addr).await.expect("connect"));
...
const STREAMFY_WASM_ARRAY_MAP_ARRAY: &str = "streamfy_smartmodule_array_map_array";

#[streamfy_future::test(ignore)]
async fn test_stream_fetch_array_map_adhoc() {
    adhoc_test(
        "test_stream_fetch_array_map_adhoc",
        STREAMFY_WASM_ARRAY_MAP_ARRAY,
...
const STREAMFY_WASM_FILTER_MAP: &str = "streamfy_smartmodule_filter_map";

#[streamfy_future::test(ignore)]
async fn test_stream_fetch_filter_map_adhoc() {
    adhoc_test(
        "test_stream_fetch_filter_map_adhoc",
        STREAMFY_WASM_FILTER_MAP,
...
const STREAMFY_WASM_FILTER_WITH_PARAMETERS: &str = "streamfy_smartmodule_filter_param";

#[streamfy_future::test(ignore)]
async fn test_stream_fetch_filter_with_params_adhoc() {
    adhoc_test(
        "test_stream_fetch_filter_with_params_adhoc",
        STREAMFY_WASM_FILTER_WITH_PARAMETERS,
        SmartModuleKind::Filter,
...
async fn test_stream_metrics() {
    ...
    }
    {
        ...
        request.header.set_client_id("streamfy_connector");
        let mut stream = client_socket
    ...
    {
        let wasm = zip(read_wasm_module(STREAMFY_WASM_FILTER));
        let smartmodule = SmartModuleInvocation {
            wasm: SmartModuleInvocationWasm::AdHoc(wasm),
            kind: SmartModuleKind::Filter,
            name: Some(STREAMFY_WASM_FILTER.to_string()),
            ..Default::default()
        };
        let mut request = RequestMessage::new_request(
            DefaultStreamFetchRequest::builder()
        ...
        request.header.set_client_id("streamfy_connector2");
        let mut stream = client_socket
            .create_stream(request, 10)
            .await
            .expect("create stream");
        ...
        if let Some(filter_metrics) = metrics_sm.get(STREAMFY_WASM_FILTER) {
            assert_eq!(filter_metrics.bytes_in(), 24);
            assert_eq!(filter_metrics.records_out(), 1);
            assert_eq!(filter_metrics.invocation_count(), 1);
        } else {
            panic!("SmartModule metrics not found for {STREAMFY_WASM_FILTER}");
        }
    }
...
const STREAMFY_WASM_FILTER_WITH_LOOKBACK: &str = "streamfy_smartmodule_filter_lookback";

#[streamfy_future::test(ignore)]
async fn test_stream_fetch_filter_lookback() {
    predefined_test(
        "test_stream_fetch_filter_lookback",
        STREAMFY_WASM_FILTER_WITH_LOOKBACK,
        SmartModuleKind::Filter,
