...
use streamfy::{SmartModuleInvocation, SmartModuleInvocationWasm, SmartModuleKind};
use streamfy_controlplane::replica::Replica;
use streamfy_smartmodule::{Record, dataplane::smartmodule::Lookback};
use streamfy_storage::{FileReplica, iterators::FileBatchIterator};
use tracing::debug;
...
use streamfy_controlplane_metadata::topic::{
    CompressionAlgorithm, Deduplication, Bounds, Filter, Transform,
};
use streamfy_future::timer::sleep;
use streamfy_socket::{MultiplexerSocket, StreamfySocket};
...
async fn test_produce_metrics() {
    ...
    }
    {
        ...
        let _ = client_socket
            .send_and_receive(
                RequestMessage::new_request(produce_request).set_client_id("streamfy_connector"),
            )
            .await
            .expect("send offset");
...
async fn test_produce_basic_with_smartmodule_with_lookback() {
    ...
    spu_config.log.base_dir = test_path;
    let ctx = GlobalContext::new_shared_context(spu_config);
    load_wasm_module(&ctx, STREAMFY_WASM_FILTER_WITH_LOOKBACK);
    let mut smartmodule = SmartModuleInvocation {
        wasm: SmartModuleInvocationWasm::Predefined(STREAMFY_WASM_FILTER_WITH_LOOKBACK.to_owned()),
        kind: SmartModuleKind::Filter,
        params: Default::default(),
        name: Some(STREAMFY_WASM_FILTER_WITH_LOOKBACK.to_owned()),
    };
...
const STREAMFY_WASM_DEDUPLICATION_FILTER: &str = "streamfy_smartmodule_filter_hashset";
...
async fn test_produce_with_deduplication() {
    ...
    spu_config.log.base_dir = test_path;
    let ctx = GlobalContext::new_shared_context(spu_config);
    load_wasm_module(&ctx, STREAMFY_WASM_DEDUPLICATION_FILTER);
    ...
    let deduplication = Deduplication {
        ...
        filter: Filter {
            transform: Transform {
                uses: STREAMFY_WASM_DEDUPLICATION_FILTER.to_owned(),
