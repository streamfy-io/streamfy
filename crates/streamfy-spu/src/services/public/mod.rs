...
use streamfy_spu_schema::server::mirror::StartMirrorRequest;
use tracing::{info, debug, trace, instrument};
use futures_util::StreamExt;
use anyhow::Result;

use streamfy_socket::StreamfySocket;
use streamfy_service::{StreamfyApiServer, StreamfyService, ConnectInfo, call_service};
use streamfy_spu_schema::server::SpuServerRequest;
...
pub(crate) type SpuPublicServer<A> =
    StreamfyApiServer<SpuServerRequest, SpuServerApiKey, SpuAuthGlobalContext<A>, PublicService<A>>;
    ...
    <A as Authorization>::Context: Send + Sync,
{
    ...
    StreamfyApiServer::new(addr, auth_ctx, PublicService::<A>::new())
}
...
#[async_trait]
impl<A> StreamfyService for PublicService<A>
where
...
async fn send_private_request_to_leader<R: Request>(
    ...
    let mut socket = StreamfySocket::connect(&leader_endpoint)
        .await
