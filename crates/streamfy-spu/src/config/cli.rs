...
use streamfy_types::defaults::SPU_PEER_MAX_BYTES;

use super::SpuConfig;

/// cli options
#[derive(Debug, Default, Parser)]
#[command(name = "streamfy-spu", about = "Streaming Processing Unit")]
pub struct SpuOpt {
...
impl SpuOpt {
    ...
    fn try_build_tls_acceptor(&self) -> Result<Option<TlsAcceptor>> {
        ...
        let acceptor = if tls_config.enable_client_cert {
            ...
            streamfy_future::rust_tls::AcceptorBuilder::with_safe_defaults()
...
fn find_spu_id_from_env() -> Result<SpuId> {
    use std::env;
    use streamfy_types::defaults::FLV_SPU_ID;
