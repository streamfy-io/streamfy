...
use streamfy_spu_schema::server::smartmodule::{SmartModuleInvocation, SmartModuleInvocationWasm};
use streamfy_storage::ReplicaStorage;
use streamfy_storage::iterators::{FileBatch, FileBatchIterator, FileRecordIterator, RecordItem};
...
async fn lookback_last_iterator<R: ReplicaStorage>(
    ...
    let slice = replica
        .read_records(offset, u32::MAX, streamfy::Isolation::ReadCommitted)
        .await?;
...
async fn read_batches_by_age<R: ReplicaStorage>(
    ...
    loop {
        ...
        let slice = replica
            .read_records(offset - 1, u32::MAX, streamfy::Isolation::ReadCommitted)
            .await?;
        let Some(file_slice) = slice.file_slice else {
            trace!(?slice);
            break;
