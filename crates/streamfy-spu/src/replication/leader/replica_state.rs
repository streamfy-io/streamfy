...
use streamfy_controlplane::{replica::Replica, sc_api::update_lrs::LrsRequest};
use tracing::{debug, error, warn};
use tracing::instrument;
use async_lock::RwLock;
use anyhow::{Result, Context};

use streamfy_protocol::record::{RecordSet, Offset, ReplicaKey, RawRecords, Batch};
use streamfy_controlplane_metadata::partition::{PartitionMirrorConfig, PartitionStatus, ReplicaStatus};
use streamfy_storage::{FileReplica, ReplicaStorage, OffsetInfo, ReplicaStorageConfig};
use streamfy_types::{
    event::offsets::{SharedOffsetPublisher, WeakSharedOffsetPublisher, TOPIC_DELETED},
    SpuId,
};
use streamfy_spu_schema::{Isolation, COMMON_VERSION};
...
mod test_leader {
    ...
    use streamfy_storage::{ReplicaStorage, ReplicaStorageConfig, OffsetInfo, ReplicaSlice};
    use streamfy_protocol::record::Offset;
    use streamfy_protocol::link::ErrorCode;
    use streamfy_protocol::record::BatchRecords;
    use streamfy_protocol::fixture::create_raw_recordset;
    ...
    impl ReplicaStorage for MockStorage {
        ...
        async fn write_recordset<R: BatchRecords>(
            &mut self,
            records: &mut streamfy_protocol::record::RecordSet<R>,
        ...
        async fn update_high_watermark(
            &mut self,
            offset: Offset,
        ) -> Result<bool, streamfy_storage::StorageError> {
            self.pos.hw = offset;
            Ok(true)
        }
        ...
        async fn remove(&self) -> Result<(), streamfy_storage::StorageError> {
            todo!()
        }
    }
