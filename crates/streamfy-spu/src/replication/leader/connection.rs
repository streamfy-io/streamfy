...
use streamfy_storage::OffsetInfo;
use streamfy_socket::{StreamfySink, SocketError, StreamfyStream};
...
impl FollowerHandler {
    ...
    async fn dispatch(mut self, sink: StreamfySink, stream: StreamfyStream) {
    ...
    async fn update_from_leaders(&mut self, sink: &mut StreamfySink) -> Result<(), SocketError> {
