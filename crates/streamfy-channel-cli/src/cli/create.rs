...
pub struct CreateOpt {
    ...
    /// Path to streamfy binary for channel
    binary_path: Option<PathBuf>,
    /// Path to streamfy extensions directory for channel
    ...
    pub async fn process(&self) -> Result<()> {
        ...
        if let Some(channel_name) = &self.channel {
            let cli_config_path = if let Some(path) = &self.config {
                ...
                StreamfyChannelConfig::default_config_location()
            };

            // Open file
            let mut config =
                if let Ok(load_config) = StreamfyChannelConfig::from_file(cli_config_path) {
            ...
            let binary_path = if let Some(binary_path) = &self.binary_path {
                ...
                // Default to ~/.streamfy/bin/streamfy-<channel>
                let mut p = home.clone();
                p.push("bin");
                p.push(format!("streamfy-{channel_name}"));
                p
            };
            ...
            let extensions_path = if let Some(extensions_path) = &self.extensions_path {
                ...
                // Default to ~/.streamfy/bin/extensions-<channel>
                let mut p = home.clone();
                p.push("bin");
            ...
            let mut new_channel_info = StreamfyChannelInfo::default();
            ...
            if !binary_path.exists() {
                ...
                install_channel_streamfy_bin(channel_name.to_string(), &config, version).await?;
            }
