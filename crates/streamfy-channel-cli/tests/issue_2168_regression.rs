// In this test which resolves [issue_2168](https://github.com/streamfy/streamfy/issues/2168),
...
// invalid version to the channel config file (~/.streamfy/config)
//
// To do so, we create a random version name that is likely to not already be in the config file.
// We verify this by running `streamfy-channel version list` and ensuring the random channel does not
// already exist.
//
// If it does not exist, we attempt to create the version with `streamfy-channel
...
//
// Finally, to clean up, we attempt to run `streamfy-channel version delete <random_channel_name>`
// just in case the version was indeed written to the file.
// This should never succeed and we assert that.
//
// streamfy-channel is deprecated, minimal maintenance ignore this test
#[ignore]
#[test]
fn issue_2168_regression_test() {
    ...
    if env::var("CI").is_ok() {
        ...
        let mut cmd = Command::cargo_bin("streamfy-channel").expect("streamfy-channel binary");
