...
pub enum Error {
    #[error("Invalid Streamfy Channel \"{0}\"")]
    InvalidChannel(String),
}

/// Package Set Channels based on Streamfy Channels
...
/// Streamfy Version Manager Package for a specific architecture and version.
#[derive(Clone, Debug, Deserialize, Serialize, PartialEq, Eq)]
...
impl From<PackageSetRecord> for PackageSet {
    fn from(value: PackageSetRecord) -> Self {
        let streamfy_artifact = value.artifacts.iter().find(|art| art.name == "streamfy");
        let streamfy_version = streamfy_artifact
            .map(|art| art.version.clone())
            .unwrap_or_else(|| Version::new(0, 0, 0));

        PackageSet {
            pkgset: streamfy_version,
            arch: value.arch,
                ...
                PackageSet {
                    ...
                    artifacts: vec![Artifact {
                        ...
                        download_url: String::from(
                            "https://packages.streamfy.io/streamfy-cloud/aarch64-apple-darwin/0.2.19",
                        ),
                        sha256_digest: None,
                    }],
                },
