...
use streamfy_controlplane_metadata::smartmodule::SmartModulePackageKey;
use streamfy_controlplane_metadata::smartmodule::SmartModuleVisibility;
use streamfy_controlplane_metadata::smartmodule as smpkg;
...
impl PackageMeta {
    ...
    ///
    /// Eg: `streamfy-example-0.0.1`
    pub fn id(&self) -> Result<String> {
        let streamfy_semver = StreamfySemVersion::parse(&self.version)
            .map_err(|err| HubError::SemVerError(err.to_string()))?;
        let package_key = SmartModulePackageKey {
            name: self.name.clone(),
            group: Some(self.group.clone()),
            version: Some(streamfy_semver),
        };

        Ok(package_key.store_id())
    }

    /// Retrives the package name from this package. Eg: `streamfy/example/0.0.1`
    pub fn pkg_name(&self) -> String {
        format!("{}/{}@{}", self.group, self.name, self.version)
    }

    /// Retrives the S3's object name from this package. Eg: `streamfy/example-0.0.1.ipkg`
    ...
    pub fn update_from_smartmodule_toml(&mut self, fpath: &str) -> Result<()> {
        ...
        // needed for streamfy sm download
...
fn builds_obj_key_from_package_name() {
    let pkg_names = [
        "streamfy/example@0.0.1",
        "streamfy/example-sm@0.1.0",
        "streamfy/json-sql@0.0.2",
        "streamfy/test@0.1.0",
        "streamfy/hub-cli@0.1.0",
        "streamfy/test-cli@0.1.0",
        "streamfy/regex@0.0.1",
    ];
    let obj_paths = [
        "streamfy/example-0.0.1.ipkg",
        "streamfy/example-sm-0.1.0.ipkg",
        "streamfy/json-sql-0.0.2.ipkg",
        "streamfy/test-0.1.0.ipkg",
        "streamfy/hub-cli-0.1.0.ipkg",
        "streamfy/test-cli-0.1.0.ipkg",
        "streamfy/regex-0.0.1.ipkg",
    ];
...
fn hub_package_id() {
    ...
    assert_eq!("example-streamfy-0.0.1", pm.id().unwrap());
}
