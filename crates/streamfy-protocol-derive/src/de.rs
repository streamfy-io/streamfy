...
pub(crate) fn generate_decode_trait_impls(input: &DeriveItem) -> TokenStream {
    match &input {
        DeriveItem::Struct(kf_struct, attrs) => {
            ...
            let generics = add_bounds(kf_struct.generics().clone(), attrs, StreamfyBound::Decoder);
            let (impl_generics, ty_generics, where_clause) = generics.split_for_impl();
            quote! {
                impl #impl_generics streamfy_protocol::Decoder for #ident #ty_generics #where_clause {
                    fn decode<T>(&mut self, src: &mut T,version: streamfy_protocol::Version) -> ::std::result::Result<(),std::io::Error> where T: streamfy_protocol::bytes::Buf {
        ...
        DeriveItem::Enum(kf_enum, attrs) => {
            ...
            let generics = add_bounds(kf_enum.generics.clone(), attrs, StreamfyBound::Decoder);
            let (impl_generics, ty_generics, where_clause) = generics.split_for_impl();
            ...
            let res = quote! {
                impl #impl_generics streamfy_protocol::Decoder for #ident #ty_generics #where_clause {
                    fn decode<T>(&mut self, src: &mut T,version: streamfy_protocol::Version) -> Result<(),std::io::Error> where T: streamfy_protocol::bytes::Buf {
                        #enum_tokens
                        Ok(())
                    }
                }
...
fn generate_try_enum_from_kf_enum(
    ...
    // If #[streamfy(encode_discriminant)] is used, this is an int-enum
...
pub(crate) fn generate_default_trait_impls(input: &DeriveItem) -> TokenStream {
    match &input {
        DeriveItem::Struct(kf_struct, attrs) => {
            ...
            let generics = add_bounds(kf_struct.generics().clone(), attrs, StreamfyBound::Default);
            let (impl_generics, ty_generics, where_clause) = generics.split_for_impl();
            quote! {
