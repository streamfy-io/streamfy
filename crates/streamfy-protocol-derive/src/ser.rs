...
use crate::ast::r#struct::StreamfyStructProps;
use crate::ast::{
    DeriveItem, container::ContainerAttributes, r#enum::EnumProp, r#enum::FieldKind,
    prop::NamedProp,
};
use crate::ast::{StreamfyBound, add_bounds};
...
pub(crate) fn generate_encode_trait_impls(input: &DeriveItem) -> TokenStream {
    match &input {
        DeriveItem::Struct(kf_struct, attrs) => {
            ...
            let generics = add_bounds(kf_struct.generics().clone(), attrs, StreamfyBound::Encoder);
            ...
            quote! {
                impl #impl_generics streamfy_protocol::Encoder for #ident #ty_generics #where_clause {
                    fn encode<T>(&self, dest: &mut T, version: streamfy_protocol::Version) ->  ::std::result::Result<(),std::io::Error> where T: streamfy_protocol::bytes::BufMut {
                        #trace_encode
        ...
        DeriveItem::Enum(kf_enum, attrs) => {
            ...
            let generics = add_bounds(kf_enum.generics.clone(), attrs, StreamfyBound::Encoder);
            let (impl_generics, ty_generics, where_clause) = generics.split_for_impl();
            ...
            quote! {
                impl #impl_generics streamfy_protocol::Encoder for #ident #ty_generics #where_clause {
                    fn encode<T>(&self, dest: &mut T, version: streamfy_protocol::Version) -> Result<(),std::io::Error> where T: streamfy_protocol::bytes::BufMut {
                        #trace_encode
...
fn parse_struct_props_encoding(
    props: &StreamfyStructProps,
    struct_ident: &Ident,
    attr: &ContainerAttributes,
) -> TokenStream {
    match props {
        StreamfyStructProps::Named(named_props) => {
            parse_struct_named_props_encoding(named_props, struct_ident, attr)
        }
        StreamfyStructProps::Unnamed(unnamed_props) => {
