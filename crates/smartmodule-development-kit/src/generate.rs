...
const STREAMFY_SMARTMODULE_CRATE_NAME: &str = "streamfy-smartmodule";
const STREAMFY_SMARTMODULE_REPO: &str = "https://github.com/streamfy-io/streamfy.git";
...
pub struct GenerateCmd {
    ...
    /// URL of git repo to include in generated Cargo.toml. Repo used for `streamfy-smartmodule` dependency.
    /// Using this option is discouraged. The default value is recommended.
    #[arg(
        long,
        hide_short_help = true,
    ...
    /// Local filepath to include in generated Cargo.toml. Path used for streamfy-smartmodule dependency.
    /// Using this option is discouraged. The default value is recommended.
    #[arg(
        long,
        hide_short_help = true,
    ...
    /// Public version of `streamfy-smartmodule` from crates.io. Defaults to latest.
    /// Using this option is discouraged. The default value is recommended.
    #[arg(
        long,
        hide_short_help = true,
    ...
    /// Using this option will always choose the Streamfy repo as source for templates and dependencies
    ...
    pub(crate) fn process(self) -> Result<()> {
        ...
        let sm_dep_source = if let Some(user_version) = self.sm_crate_version {
            ...
            if let Some(branch) = self.sm_repo_branch {
                ...
                CargoSmDependSource::Git(STREAMFY_SMARTMODULE_REPO.to_string())
            }
            ...
            let latest_sm_crate_info =
                Info::new().fetch(vec![STREAMFY_SMARTMODULE_CRATE_NAME], &InfoOpts::default())?;
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            SmdkTemplateValue::SmCargoDependency(dependency) => {
                write!(f, "streamfy-smartmodule-cargo-dependency={dependency}")
            }
...
impl SmdkTemplateUserValues {
    ...
    fn with_smart_module_cargo_dependency(
        ...
        if let Some(d) = dependency {
            debug!("User provided streamfy-smartmodule Cargo.toml value: {d:#?}");
...
mod test {
    ...
    use super::STREAMFY_SMARTMODULE_REPO;
    ...
    fn test_cargo_dependency_values() {
        ...
        let test_template_values = vec![
            ...
            CargoSmDependSource::Git(STREAMFY_SMARTMODULE_REPO.to_string()),
            CargoSmDependSource::GitBranch {
                url: STREAMFY_SMARTMODULE_REPO.to_string(),
            ...
            match value {
                ...
                SmdkTemplateValue::SmCargoDependency(_) => {
                    assert_eq!(
                        ...
                        .to_string(),
                        "streamfy-smartmodule-cargo-dependency=\"0.1.0\""
                    );
                }
