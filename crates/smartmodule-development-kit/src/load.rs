...
/// Load SmartModule into Streamfy cluster
#[derive(Debug, Parser)]
pub struct LoadCmd {
    #[arg(long)]
    name: Option<String>,
...
impl LoadCmd {
    pub(crate) fn process(self) -> Result<()> {
        ...
        let streamfy_config = self.target.clone().load()?;

        if let Err(e) = run_block_on(create(streamfy_config, spec, sm_id, self.dry_run)) {
            eprintln!("{e}");
            std::process::exit(1);
        }
        Ok(())
...
async fn create(
    config: StreamfyClusterConfig,
    spec: SmartModuleSpec,
    id: String,
    dry_run: bool,
) -> Result<()> {
    println!("Trying connection to streamfy {}", config.endpoint);
