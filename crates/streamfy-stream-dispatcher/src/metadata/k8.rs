        ...
        S: K8ExtendedSpec,
    {
        ...
        let k8_object = K8MetadataClient::patch_status(
            ...
            PatchMergeType::Apply(ApplyOptions {
                force: true,
                field_manager: Some(String::from("streamfy")),
            }),
        )
        .await?;
        let multi_namespace_context = matches!(namespace, NameSpace::All);
...
mod tests {
    ...
    async fn test_watch_stream_since_start() {
        ...
        let updates = stream
            .take_until(streamfy_future::timer::sleep(Duration::from_secs(2)))
    ...
    async fn test_apply_with_parent() {
        ...
        let ctx = streamfy_stream_model::store::k8::K8MetadataContext::new(meta);
