...
use crate::{Encoder as StreamfyEncoder, Decoder as StreamfyDecoder, Version};

/// Implement Streamfy Encoding
/// First 4 bytes are size of the message.  Then total buffer = 4 + message content
///
#[derive(Debug, Default)]
pub struct StreamfyCodec {}

/// Type used as input by the [`StreamfyCodec`] encoder implementation.
/// Contains the data of the message and the [`crate::core:Version`].
pub type StreamfyCodecData<T> = (T, Version);
...
impl Decoder for StreamfyCodec {
    type Item = BytesMut;
    type Error = IoError;
...
/// Implement encoder for Streamfy Codec
/// This is straight pass thru, actual encoding is done file slice
impl Encoder<Bytes> for StreamfyCodec {
    type Error = IoError;
...
/// Implement encoder for Streamfy Codec
impl<T: StreamfyEncoder> Encoder<StreamfyCodecData<T>> for StreamfyCodec {
    type Error = IoError;

    fn encode(&mut self, src: StreamfyCodecData<T>, buf: &mut BytesMut) -> Result<(), IoError> {
        let (src, version) = src;
...
mod test {
    ...
    use streamfy_future::net::TcpStream;
    use streamfy_future::timer::sleep;
    use crate::{Encoder, Decoder};

    use super::StreamfyCodec;
    ...
    async fn run_server_raw_data<T: Encoder>(data: T, addr: &SocketAddr) -> Result<(), Error> {
        ...
        if let Some(stream) = incoming.next().await {
            ...
            tcp_stream.write_all(&encoded_data).await?;
            streamfy_future::timer::sleep(time::Duration::from_millis(10)).await;
            tcp_stream.write_all(&buf2).await?;
        }
        streamfy_future::timer::sleep(time::Duration::from_millis(50)).await;
    ...
    async fn run_server_object<T: Encoder + Clone>(
        ...
        if let Some(stream) = incoming.next().await {
            ...
            let framed = Framed::new(tcp_stream.compat(), StreamfyCodec {});
