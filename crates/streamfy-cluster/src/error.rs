...
pub enum ClusterError {
    /// An error occurred while trying to install Streamfy on Kubernetes
    #[error("Failed to install Streamfy on Kubernetes: {0}")]
    InstallK8(#[from] K8InstallError),
    /// An error occurred while trying to install Streamfy locally
    #[error("Failed to install Streamfy locally: {0}")]
    InstallLocal(#[from] LocalInstallError),
    /// An error occurred while trying to install Streamfy system charts
    #[error("Failed to install Streamfy system charts: {0}")]
    InstallSys(#[from] ChartInstallError),
    /// An error occurred while trying to uninstall Streamfy
    #[error("Failed to uninstall Streamfy: {0}")]
    Uninstall(#[from] UninstallError),
    #[error("Progress Error: {0}")]
    ProgressError(#[from] TemplateError),
}

/// Errors that may occur while trying to install Streamfy on Kubernetes
#[derive(thiserror::Error, Debug)]
pub enum K8InstallError {
    ...
    /// Failed to update Streamfy cluster
    ...
    SPUTimeout,
    /// Unable to find Streamfy SC service in Kubernetes
    #[error("Unable to detect Streamfy SC K8 service")]
...
/// Errors that may occur while trying to install Streamfy locally
#[derive(thiserror::Error, Debug)]
pub enum LocalInstallError {
    ...
    /// An error occurred with the Streamfy client.
    #[error("Streamfy client error: {0}")]
    StreamfyError(#[from] StreamfyError),
    /// An error occurred with the Kubernetes config.
    #[error("Kubernetes config error: {0}")]
    K8ConfigError(#[from] K8ConfigError),
    /// An error occurred while running helm.
...
/// Errors that may occur while trying to unintsall Streamfy
#[derive(thiserror::Error, Debug)]
