...
pub async fn kill_local_processes(pb: &ProgressRenderer) -> Result<()> {
    pb.set_message("Uninstalling streamfy local components");
    ...
    kill_proc("streamfy", Some(&["cluster".into(), "run".into()]));
    kill_proc("streamfy", Some(&["run".into()]));
    kill_proc("streamfy-run", None);
