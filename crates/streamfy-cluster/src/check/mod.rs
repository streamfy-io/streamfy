...
pub enum ClusterCheckError {
    ...
    /// local streamfy exists
    #[error("Local Streamfy running")]
    LocalClusterExists,
...
pub enum RecoverableCheck {
    /// The streamfy-sys chart is not installed
    #[error("Missing Streamfy system charts.")]
    MissingSystemChart,

    #[error("Streamfy system charts are not up to date.")]
    UpgradeSystemChart,
}
...
impl CheckSuggestion for RecoverableCheck {
    fn suggestion(&self) -> Option<String> {
        let suggestion = match self {
            Self::MissingSystemChart => "Run 'streamfy cluster start --sys'",
            Self::UpgradeSystemChart => "Run 'streamfy cluster start --sys'",
        };
...
pub enum UnrecoverableCheckStatus {
    ...
    /// There are multiple streamfy-sys's installed
    #[error("Cannot have multiple versions of streamfy-sys installed")]
    MultipleSystemCharts,

    #[error("Streamfy chart is already installed")]
    AlreadyInstalled,
    ...
    #[error("Local Streamfy cluster still running")]
    ExistingLocalCluster,

    #[error(
        "Local Streamfy cluster wasn't deleted. Use 'resume' to resume created cluster or 'delete' before starting a new one"
    )]
    CreateLocalConfigError,

    /// The installed version of the local cluster is incompatible
    #[error(
        "Check Versions match failed: cannot resume a {installed} cluster with streamfy version {required}.\nShutdown the cluster with \"streamfy cluster shutdown\" and use \"streamfy cluster upgrade\" to update to the new version"
    )]
...
/// Streamfy Cluster component
#[derive(Debug, Hash, PartialEq, Eq)]
pub enum StreamfyClusterComponent {
    Helm,
    Kubernetes,
    K8Version,
    SysChart,
...
pub trait ClusterCheck: Debug + 'static + Send + Sync {
    ...
    fn component(&self) -> Option<StreamfyClusterComponent> {
        None
    }
...
impl ClusterCheck for ActiveKubernetesCluster {
    ...
    fn component(&self) -> Option<StreamfyClusterComponent> {
        Some(StreamfyClusterComponent::Kubernetes)
    }
...
impl ClusterCheck for K8Version {
    ...
    fn required_components(&self) -> Vec<StreamfyClusterComponent> {
        vec![StreamfyClusterComponent::Kubernetes]
    }

    fn component(&self) -> Option<StreamfyClusterComponent> {
        Some(StreamfyClusterComponent::K8Version)
    }
...
impl ClusterCheck for HelmVersion {
    ...
    fn component(&self) -> Option<StreamfyClusterComponent> {
        Some(StreamfyClusterComponent::Helm)
    }
...
impl ClusterCheck for SysChartCheck {
    ...
    async fn perform_check(&self, _pb: &ProgressRenderer) -> CheckResult {
        ...
        if sys_charts.is_empty() {
            ...
            if existing_platform_version == self.platform_version {
                Ok(CheckStatus::pass("Streamfy system charts are installed"))
            } else {
                Ok(CheckStatus::AutoFixableError {
                    message: format!(
    ...
    fn required_components(&self) -> Vec<StreamfyClusterComponent> {
        vec![
            StreamfyClusterComponent::Helm,
            StreamfyClusterComponent::Kubernetes,
        ]
    }

    fn component(&self) -> Option<StreamfyClusterComponent> {
        Some(StreamfyClusterComponent::SysChart)
    }
...
impl ClusterAutoFix for InstallSysChart {
    async fn attempt_fix(&self, _render: &ProgressRenderer) -> Result<String> {
        debug!(
            "Fixing by installing Streamfy sys chart with config: {:#?}",
            &self.config
        );
        ...
        Ok(format!(
            "Streamfy Sys chart {} is installed",
            self.platform_version
        ))
    }
}
...
impl ClusterAutoFix for UpgradeSysChart {
    async fn attempt_fix(&self, _render: &ProgressRenderer) -> Result<String> {
        debug!(
            "Fixing by updating Streamfy sys chart with config: {:#?}",
            &self.config
        );
        ...
        Ok(format!(
            "Streamfy sys chart is upgraded to: {}",
            self.platform_version
        ))
    }
}
...
impl ClusterCheck for AlreadyInstalled {
    /// Checks that Streamfy is not already installed
    async fn perform_check(&self, _pb: &ProgressRenderer) -> CheckResult {
        let helm = HelmClient::new()?;
...
impl ClusterCheck for LocalClusterCheck {
    async fn perform_check(&self, _pb: &ProgressRenderer) -> CheckResult {
        ...
        Ok(CheckStatus::pass("Local Streamfy is not installed"))
    }

    fn label(&self) -> &str {
        "Streamfy Local Installation"
    }
}
...
impl ClusterCheck for CleanLocalClusterCheck {
    async fn perform_check(&self, _pb: &ProgressRenderer) -> CheckResult {
        ...
        Ok(CheckStatus::pass(
            "Previous local streamfy installation not found",
        ))
    }

    fn label(&self) -> &str {
        "Clean Streamfy Local Installation"
    }
}
...
impl ClusterChecker {
    ...
    ///
    /// ```
    /// # use streamfy_cluster::ClusterChecker;
    ...
    pub async fn run(self, pb_factory: &ProgressBarFactory, fix_recoverable: bool) -> Result<bool> {
        ...
        let mut components: HashSet<StreamfyClusterComponent> = HashSet::new();
