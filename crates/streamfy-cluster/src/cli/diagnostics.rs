...
use streamfy::metadata::{topic::TopicSpec, partition::PartitionSpec, spg::SpuGroupSpec, spu::SpuSpec};
...
const STREAMFY_PROCESS_NAME: &str = "streamfy";
...
impl DiagnosticsOpt {
    pub async fn process(self) -> Result<()> {
        ...
        let temp_dir = tempfile::Builder::new()
            .prefix("streamfy-diagnostics")
            .tempdir()?;
        let temp_path = temp_dir.path();

        let spu_specs = match self.copy_streamfy_specs(temp_path).await {
            Ok(specs) => specs,
            Err(err) => {
                eprintln!("error copying streamfy specs: {err:#?}");
                vec![]
            }
        };

        // write internal streamfy cluster internal state
    ...
    fn copy_kubernetes_logs(&self, kubectl: &Path, dest_dir: &Path) -> Result<()> {
        ...
        .read()?;
        // Filter for only Streamfy pods
        let pods = pods
            .split(' ')
            .filter(|pod| pod.contains(STREAMFY_PROCESS_NAME))
            .collect::<Vec<_>>();
    ...
    fn copy_kubernetes_metadata(
        ...
        .read()?;
        // Filter for only Streamfy services
        let objects = objects
            .split(' ')
            .filter(|obj| !filter_streamfy || obj.contains(STREAMFY_PROCESS_NAME))
            .map(|name| name.trim())
            .collect::<Vec<_>>();
    ...
    async fn copy_streamfy_specs(&self, dest: &Path) -> Result<Vec<Metadata<SpuSpec>>> {
        use streamfy::Streamfy;

        println!("start copying streamfy specs from...");
    ...
    fn spu_disk_usage(
        ...
        let ls_cmd = match kubectl {
            Some(kct) => {
                let pod_id = format!("streamfy-spg-main-{spu}");
                ...
                cmd!(
                    ...
                    "-lh",
                    "-R",
                    format!("/var/lib/streamfy/data/spu-logs-{spu}/")
                )
            }
            None => {
    ...
    fn load(sys: &System) -> Vec<ProcessInfo> {
        ...
        for (pid, process) in sys.processes() {
            ...
            if let Some(process_name) = process_name
                && process_name.contains(STREAMFY_PROCESS_NAME)
            {
