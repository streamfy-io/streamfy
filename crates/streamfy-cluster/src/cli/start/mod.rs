...
use streamfy_controlplane_metadata::spg::{SpuConfig, StorageConfig};
use streamfy_types::defaults::{TLS_SERVER_SECRET_NAME, TLS_CLIENT_SECRET_NAME};
...
pub fn default_log_directory() -> PathBuf {
    let base = streamfy_cli_common::install::streamfy_base_dir().unwrap_or(std::env::temp_dir());
    base.join("log")
}
