...
use streamfy::config::{TlsPolicy, TlsPaths};
...
mod tests {
    ...
    fn test_from_opt() {
        ...
        use streamfy::config::{TlsPolicy::*, TlsConfig::*};
        match (client, server) {
            (Verified(Files(client_paths)), Verified(Files(server_paths))) => {
                // Client checks
                assert_eq!(client_paths.domain, "streamfy.io");
                ...
                // Server checks
                assert_eq!(server_paths.domain, "streamfy.io");
