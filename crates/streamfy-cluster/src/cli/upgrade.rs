...
use streamfy_stream_dispatcher::metadata::{local::LocalMetadataStorage, MetadataClient};
...
impl UpgradeOpt {
    pub async fn process(mut self, platform_version: Version) -> Result<()> {
        ...
        if !self.force {
            let prompt = dialoguer::Confirm::new()
                .with_prompt(format!(
                    "Upgrade Local Streamfy cluster to version {platform_version}?"
                ))
                .interact()?;
        ...
        match installation_type {
            ...
            InstallationType::Cloud => {
                ...
                bail!(
                    "Streamfy cluster upgrade does not operate on cloud cluster \"{profile}\", use 'streamfy cloud ...' commands"
                )
            }
    ...
    async fn upgrade_local_cluster(
        ...
        pb.println(format!(
            "🚀 {}",
            format!("Upgrading Local Streamfy cluster to {platform_version}").bold(),
        ));
        ...
        pb.println(format!(
            "🎉 {}",
            format!("Successfully upgraded Local Streamfy cluster to {platform_version}").bold(),
        ));

        pb.println(format!(
            "Run: {} to start the cluster again",
            "streamfy cluster resume".bold()
        ));
        Ok(())
    }
}
