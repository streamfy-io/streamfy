...
mod error {
    ...
    pub enum LocalRuntimeError {
        ...
        /// Attempted to launch local cluster without streamfy-run
        #[error("Local cluster requires the streamfy-run plugin to be installed")]
        MissingStreamfyRunner,
...
mod process {
    ...
    use streamfy::config::{TlsConfig, TlsPaths};

    use crate::tls_config_to_cert_paths;

    use super::LocalRuntimeError;

    pub trait StreamfyLocalProcess {
