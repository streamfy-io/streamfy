...
use streamfy_controlplane_metadata::spg::SpuConfig;
use streamfy_sc_schema::objects::CommonCreateRequest;
use streamfy_types::defaults::TLS_CLIENT_SECRET_NAME;
use streamfy_types::defaults::TLS_SERVER_SECRET_NAME;
...
use streamfy::metadata::spu::SpuSpec;
use streamfy::config::{TlsPolicy, TlsConfig, TlsPaths, ConfigFile};
...
const DEFAULT_REGISTRY: &str = "ghcr.io/streamfy-io";
...
/// Describes how to install Streamfy onto Kubernetes
...
pub struct ClusterConfig {
    ...
    /// Sets the Kubernetes namespace to install Streamfy into.
    ///
    ...
    ///
    /// ```
    /// # use streamfy_cluster::{ClusterConfig, ClusterConfigBuilder, ClusterError};
    ...
    namespace: String,
    /// Sets the docker image tag of the Streamfy image to install.
    ///
    ...
    /// official published Streamfy images.
    ///
    /// # Example
    ///
    /// Suppose you would like to install version `0.6.0` of Streamfy from
    /// a container registry, where the image is tagged as `streamfy-io/streamfy:0.6.0`.
    ...
    /// Sets the container image registry prefix used to download Streamfy images.
    ///
    /// This defaults to `ghcr.io/streamfy-io` to pull from the GitHub
    /// Container Registry. This can be used to specify a private registry or a
    /// local registry as a source of Streamfy images.
    ///
    /// # Example
    ///
    ...
    /// ```
    ///
    /// Suppose you tagged your image as `streamfy-io/streamfy:0.1.0` and pushed it
    /// to your `localhost:5000` registry. Your image is now located at
    /// `localhost:5000/streamfy`. You can specify that to the installer like so:
    ///
    ...
    /// # fn example(builder: &mut ClusterConfigBuilder) -> anyhow::Result<()> {
    /// let config = builder
    ///     .image_registry("localhost:5000/streamfy")
    ///     .build()?;
    /// # Ok(())
    /// # }
    /// ```
    ///
    /// Then, when you use `installer.install_streamfy()`, it will pull the images
    ...
    /// Sets a specific version of the Streamfy helm chart to install.
    ...
    /// Whether to save a profile of this installation to `~/.streamfy/config`. Defaults to `false`.
    ///
    /// # Example
    ///
    /// ```
    ...
    save_profile: bool,
    /// Whether to install the `streamfy-sys` chart in the full installation. Defaults to `true`.
    ///
    /// # Example
    ///
...
impl ClusterConfig {
    ...
    ///
    /// ```
    /// # use streamfy_cluster::ClusterConfig;
    /// use semver::Version;
...
impl ClusterConfigBuilder {
    ...
    /// Sets a local helm chart location to search for Streamfy charts.
    ///
    /// This is often desirable when developing for Streamfy locally and making
    ...
    /// of both the `streamfy-app` and `streamfy-sys` charts.
    ///
    ...
    /// Sets a remote helm chart location to search for Streamfy charts.
    ///
    /// This is the default case, with the default location being `https://charts.streamfy.io`,
    /// where official Streamfy helm charts are located. Remote helm charts are expected
    /// to be a valid URL.
    ///
    ...
    /// # fn example(builder: &mut ClusterConfigBuilder) -> anyhow::Result<()> {
    /// let config = builder
    ///     .remote_chart("https://charts.streamfy.io")
    ///     .build()?;
    /// # Ok(())
    /// # }
    /// ```
    ...
    /// # fn example(builder: &mut ClusterConfigBuilder) -> anyhow::Result<()> {
    /// use std::path::PathBuf;
    /// use streamfy::config::TlsPaths;
    /// use semver::Version;
    ///
    /// let cert_path = PathBuf::from("/tmp/certs");
    /// let client = TlsPaths {
    ///     domain: "streamfy.io".to_string(),
    ...
    ///
    /// ```
    /// # use streamfy_cluster::{ClusterError, ClusterConfig};
...
/// Allows installing Streamfy on a Kubernetes cluster
///
/// Streamfy's Kubernetes components are distributed as [Helm Charts],
...
///
/// If you want to try out Streamfy on Kubernetes, you can use [Minikube]
...
///
/// ```
/// # use streamfy_cluster::{ClusterInstaller, ClusterConfig};
/// # async fn example() -> anyhow::Result<()> {
/// use semver::Version;
/// let config = ClusterConfig::builder(Version::parse("0.7.0-alpha.1").unwrap()).build()?;
/// let installer = ClusterInstaller::from_config(config)?;
/// let _status = installer.install_streamfy().await?;
/// # Ok(())
/// # }
/// ```
///
...
impl ClusterInstaller {
    ...
    ///
    /// ```
    /// # use streamfy_cluster::{ClusterConfig, ClusterInstaller};
    ...
    /// Checks if all of the prerequisites for installing Streamfy are met
    ///
    ...
    /// Installs Streamfy according to the installer's configuration
    ///
    ...
    pub async fn install_streamfy(&self) -> Result<StartStatus> {
        ...
        let cluster_config = StreamfyClusterConfig::new(install_host_and_port.clone())
            .with_tls(self.config.client_tls_policy.clone());
        pb.set_message("🔎 Discovering Streamfy SC");
        let streamfy =
            match try_connect_to_sc(&cluster_config, &self.config.platform_version, &pb).await {
                Some(streamfy) => streamfy,
        ...
        if let Some(default_spu_group) = &self.config.default_spu_group {
            Self::create_managed_spu_group(default_spu_group, &streamfy, &pb).await?;
        }
    ...
    /// Install Streamfy Core chart on the configured cluster
    #[instrument(skip(self))]
    async fn install_app(&self) -> Result<()> {
        debug!(
            "Installing streamfy with the following configuration: {:#?}",
            &self.config
        );
        ...
        if self.config.upgrade {
            pb.set_message(format!(
                "📊 Upgrading Streamfy app chart to {}",
                self.config.platform_version
            ));
        } else {
            pb.set_message(format!(
                "📊 Installing Streamfy app chart: {}",
                self.config.platform_version
            ));
        }
        ...
        let _temp_files = if self.config.service_type == "NodePort" {
            ...
            ingress_address.insert("streamfy.io/ingress-address", access_addr);
        ...
        if self.config.upgrade {
            pb.println(format!(
                "✅ Upgrading Streamfy app chart: {}",
                self.config.platform_version
            ));
        } else {
            pb.println(format!(
                "✅ Installed Streamfy app chart: {}",
                self.config.platform_version
            ));
        }
    ...
    async fn start_sc_port_forwarding(
        ...
        let mut pf_child = std::process::Command::new("kubectl")
            ...
            .arg("port-forward")
            .arg(format!("service/{STREAMFY_SC_SERVICE}"))
    ...
    /// Looks up the external address of a Streamfy SC instance in the given namespace
    #[instrument(skip(self))]
                    ...
                    if let Some(service_watches) = service_next {
                        ...
                        for service_watch in service_watches? {
                            ...
                            if let Some(service) = service_value
                                ...
                                && service.metadata.name == STREAMFY_SC_SERVICE {
                    ...
                    if let Some(deployment_watches) = deployment_next {
                        ...
                        for deployment_watch in deployment_watches? {
                            ...
                            if let Some(deployment) = deployment_value
                                ...
                                && deployment.metadata.name == STREAMFY_SC_DEPLOYMENT {
    ...
    fn upload_tls_secrets_from_files(
        ...
        Command::new("kubectl")
            .args(["delete", "secret", "streamfy-ca", "--ignore-not-found=true"])
        ...
        Command::new("kubectl")
            .args(["create", "secret", "generic", "streamfy-ca"])
    ...
    /// Updates the Streamfy configuration with the newly installed cluster info.
