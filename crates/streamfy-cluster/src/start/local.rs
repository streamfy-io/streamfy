...
pub struct LocalConfig {
    ...
    ///
    /// ```
    /// # use streamfy_cluster::{ClusterError, LocalConfigBuilder};
    ...
    ///
    /// This is necessary because when `streamfy-cluster` is linked into any
    /// binary other than the Streamfy CLI, it needs to know how to invoke
    ...
    /// The version of the Streamfy system chart to install
    ///
...
impl LocalConfig {
    ...
    ///
    /// ```
    /// # use streamfy_cluster::LocalConfig;
    /// use semver::Version;
...
impl LocalConfigBuilder {
    ...
    ///
    /// ```
    /// # use streamfy_cluster::{ClusterError, LocalConfig};
    ...
    ///
    /// ```
    /// # use streamfy_cluster::{LocalConfig, LocalConfigBuilder, ClusterError};
    /// # fn example(builder: &mut LocalConfigBuilder) -> anyhow::Result<()> {
    /// use std::path::PathBuf;
    /// use streamfy::config::TlsPaths;
    /// use streamfy_cluster::LocalInstaller;
    /// use semver::Version;
    ///
...
/// Install streamfy cluster locally
#[derive(Debug)]
...
impl LocalInstaller {
    ...
    ///
    /// ```
    /// # use streamfy_cluster::{ClusterError, LocalInstaller, LocalConfig};
    /// # fn example() -> anyhow::Result<()> {
    /// use semver::Version;
    ...
    /// Checks if all of the prerequisites for installing Streamfy locally are met
    ...
    pub async fn install_only(&self) -> Result<StartStatus> {
        ...
        if let Some(ref client) = maybe_k8_client {
            ...
            self.launch_spu_group_read_only(&streamfy, &pb).await?;
        }

        self.confirm_spu(self.config.spu_replicas, &streamfy, &pb)
            .await?;
        ...
        self.pb_factory
            .println("🎯 Successfully installed Local Streamfy cluster");
    ...
    async fn launch_sc(
        ...
        let cluster_config = StreamfyClusterConfig::new(public_address)
    ...
    async fn launch_spu_group_read_only(
        ...
        for i in 0..count {
            pb.set_message(InstallProgressMessage::StartSPU(i + 1, count).msg());
            self.launch_spu_read_only(streamfy, i, &runtime).await?;
        }
        sleep(Duration::from_millis(500)).await;
        Ok(())
    }
    ...
    #[instrument(skip(self, cluster_manager, streamfy))]
    ...
    async fn confirm_spu(&self, spu: u16, client: &Streamfy, pb: &ProgressRenderer) -> Result<()> {
