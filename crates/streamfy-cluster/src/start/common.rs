...
pub async fn try_connect_to_sc(
    ...
    ) -> Result<Streamfy, TryConnectError> {
        match Streamfy::connect_with_config(streamfy_config).await {
            Ok(streamfy) => {
                let current_version = streamfy.platform_version();
...
pub async fn check_crd(client: SharedK8Client) -> anyhow::Result<()> {
    ...
    for i in 0..100 {
        debug!("checking streamfy crd attempt: {}", i);
        // check if spu is installed
        if let Err(err) = client.retrieve_items::<SpuSpec, _>("default").await {
            debug!("problem retrieving streamfy crd {}", err);
            sleep(Duration::from_secs(1)).await;
        } else {
            debug!("streamfy crd installed");
            return Ok(());
        }
    }

    Err(anyhow::anyhow!("Streamfy CRD not ready"))
}
