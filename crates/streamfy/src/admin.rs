...
use streamfy_sc_schema::objects::ObjectApiUpdateRequest;
use streamfy_sc_schema::objects::UpdateRequest;
use streamfy_sc_schema::UpdatableAdminSpec;
use streamfy_protocol::{Decoder, Encoder};
use streamfy_protocol::api::{Request, RequestMessage};
use streamfy_future::net::DomainConnector;
use streamfy_sc_schema::objects::{
...
use streamfy_sc_schema::{AdminSpec, DeletableAdminSpec, CreatableAdminSpec, TryEncodableFrom};
use streamfy_socket::{ClientConfig, VersionedSerialSocket, SerialFrame, MultiplexerSocket};

use crate::StreamfyClusterConfig;
use crate::config::ConfigFile;
use crate::error::anyhow_version_error;
use crate::metadata::objects::{ListResponse, ListRequest};
use crate::sync::MetadataStores;

/// An interface for managing a Streamfy cluster
///
/// Most applications will not require administrator functionality. The
/// `StreamfyAdmin` interface is used to create, edit, and manage Topics
...
/// goes for Streamfy administrators.
///
/// If you _are_ writing an application whose purpose is to manage a
/// Streamfy cluster for you, you can gain access to the `StreamfyAdmin`
/// client via the regular [`Streamfy`] client, or through the [`connect`]
/// or [`connect_with_config`] functions.
///
/// # Example
///
/// Note that this may fail if you are not authorized as a Streamfy
/// administrator for the cluster you are connected to.
///
/// ```no_run
/// # use streamfy::{Streamfy, StreamfyError};
/// # async fn do_get_admin(streamfy: &mut Streamfy) -> Result<(), StreamfyError> {
...
/// [`connect`]: ./struct.StreamfyAdmin.html#method.connect
/// [`connect_with_config`]: ./struct.StreamfyAdmin.html#method.connect_with_config
pub struct StreamfyAdmin {
...
impl StreamfyAdmin {
    ...
    /// Creates a new admin connection using the current profile from `~/.streamfy/config`
    ///
    /// This will attempt to read a Streamfy cluster configuration from
    /// your `~/.streamfy/config` file, or create one with default settings
    ...
    /// let admin = StreamfyAdmin::connect().await?;
    /// # Ok(())
    /// # }
    /// ```
    ///
    ...
    /// let config_file = ConfigFile::load_default_or_new()?;
    /// let streamfy_config = config_file.config().current_cluster().unwrap();
    /// let admin = StreamfyAdmin::connect_with_config(streamfy_config).await?;
    /// # Ok(())
    /// # }
    /// ```
