...
use streamfy_socket::ClientConfig;
...
use streamfy_protocol::record::ConsumerRecord;
use streamfy_sc_schema::errors::ErrorCode;

use crate::consumer::RetryMode;
use crate::{Streamfy, StreamfyClusterConfig, Offset};
use super::{
...
pub const SPAN_RETRY: &str = "streamfy::retry";
...
/// A default reconnect strategy that uses the Streamfy client to reconnect.
...
impl ReconnectStrategy for DefaultReconnectStrategy {
    async fn reconnect(
        ...
        let streamfy_client = Streamfy::connect_with_connector(
        ...
        let new_stream =
            ConsumerRetryStream::create_owned_stream(streamfy_client, new_config.clone()).await?;
...
impl ConsumerRetryStream {
    /// Creates an owned consumer stream from a Streamfy client.
    async fn create_owned_stream(
        streamfy: Streamfy,
        config: ConsumerConfigExt,
    ) -> Result<
        impl ConsumerStream<
            Item = std::result::Result<ConsumerRecord, streamfy_protocol::link::ErrorCode>,
        >,
    > {
        streamfy.consumer_with_config_inner(config).await
    }
    ...
    pub async fn new(
        ...
        let client_config = streamfy.client_config();
        let stream = streamfy.consumer_with_config_inner(config.clone()).await?;
...
mod tests {
    ...
    use streamfy_smartmodule::RecordData;
        ...
        T: std::marker::Unpin,
    {
        ...
        let inner = ConsumerRetryInner {
            ...
            cluster_config: super::StreamfyClusterConfig::new("localhost:9003".to_string()),
            next_offset_to_read: None,
    ...
    fn create_data(
        ...
        let mut records: Vec<_> = input
            .into_iter()
            .map(|item| streamfy_protocol::record::Record::new(RecordData::from(item.as_bytes())))
            .collect();
    ...
    async fn test_retry_stream() {
        ...
        let mut retry_stream = ConsumerRetryStream {
            inner: ConsumerRetryInner {
                client_config: Arc::new(ClientConfig::with_addr("localhost:9010".to_string())),
                cluster_config: StreamfyClusterConfig::new("localhost:9003".to_string()),
                next_offset_to_read: None,
                consumer_config: ConsumerConfigExt::builder()
        ...
        streamfy_future::task::spawn(async move {
