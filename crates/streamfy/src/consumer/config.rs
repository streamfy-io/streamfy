...
use streamfy_spu_schema::{server::smartmodule::SmartModuleInvocation, Isolation};
use streamfy_types::PartitionId;

use crate::{StreamfyError, Offset};
...
impl ConsumerConfigBuilder {
    pub fn build(&self) -> Result<ConsumerConfig> {
        let config = self.build_impl().map_err(|e| {
            StreamfyError::ConsumerConfig(format!("Missing required config option: {e}"))
        })?;
        Ok(config)
    }
}
...
impl FromStr for OffsetManagementStrategy {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s.to_lowercase().as_str() {
            ...
            _ => Err(StreamfyError::ConsumerConfig(format!(
...
impl ConsumerConfigExtBuilder {
    pub fn build(&self) -> Result<ConsumerConfigExt> {
        ...
        {
            return Err((StreamfyError::ConsumerConfig(
