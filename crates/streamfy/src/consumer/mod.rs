...
use streamfy_socket::VersionedSerialSocket;
use streamfy_spu_schema::server::consumer_offset::{
...
use streamfy_types::defaults::{
    CONSUMER_REPLICA_KEY, STREAMFY_CLIENT_MAX_FETCH_BYTES, STREAMFY_MAX_SIZE_TOPIC_NAME,
    RECONNECT_BACKOFF_FACTOR, RECONNECT_BACKOFF_MAX_DURATION, RECONNECT_BACKOFF_MIN_DURATION,
};
use streamfy_spu_schema::server::stream_fetch::{
...
pub use streamfy_protocol::record::ConsumerRecord;

pub use streamfy_protocol::record::ConsumerRecord as Record;
pub use streamfy_spu_schema::server::smartmodule::SmartModuleInvocation;
pub use streamfy_spu_schema::server::smartmodule::SmartModuleInvocationWasm;
pub use streamfy_spu_schema::server::smartmodule::SmartModuleKind;
pub use streamfy_spu_schema::server::smartmodule::SmartModuleContextData;
pub use streamfy_smartmodule::dataplane::smartmodule::SmartModuleExtraParams;
...
/// [`partition_consumer`]: struct.Streamfy.html#method.partition_consumer
    ...
    P: SpuDirectory + 'static,
{
    ...
    ///
    /// Streaming is one of the two ways to consume events in Streamfy.
    ...
    ///
    /// ```
    /// # use streamfy::{PartitionConsumer};
    /// # use streamfy::{Offset, ConsumerConfig};
    /// # mod futures {
    ...
    async fn inner_stream_batches_with_config(
        ...
        streamfy_protocol::record::Offset,
...
static MAX_FETCH_BYTES: Lazy<i32> = Lazy::new(|| {
    ...
    use streamfy_spu_schema::fetch::FetchableTopicResponse;
    use streamfy_spu_schema::fetch::FetchablePartitionResponse;
    ...
    let max_bytes: i32 = var_value.parse().unwrap_or_else(|_| {
        FetchResponse::<MemoryRecords>::default().write_size(0) as i32
            ...
            + STREAMFY_MAX_SIZE_TOPIC_NAME as i32 // using max size of topic name
            + STREAMFY_CLIENT_MAX_FETCH_BYTES
    });
    max_bytes
});
...
impl MultiplePartitionConsumer {
    ...
    ///
    /// ```
    /// # use streamfy::{MultiplePartitionConsumer};
    /// # use streamfy::{Offset, ConsumerConfig};
    /// # mod futures {
    /// #     pub use futures_util::stream::StreamExt;
    /// # }
