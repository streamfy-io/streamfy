...
use streamfy_future::net::DomainConnector;
use streamfy_sc_schema::partition::PartitionMirrorConfig;
use streamfy_sc_schema::topic::{MirrorConfig, PartitionMap, ReplicaSpec};
use streamfy_sc_schema::objects::ObjectApiWatchRequest;
use streamfy_types::PartitionId;
use streamfy_socket::{
    ClientConfig, Versions, VersionedSerialSocket, SharedMultiplexerSocket, MultiplexerSocket,
};

use crate::admin::StreamfyAdmin;
use crate::consumer::{
...
use crate::{TopicProducer, PartitionConsumer, StreamfyError, StreamfyClusterConfig};

/// An interface for interacting with Streamfy streaming
pub struct Streamfy {
    socket: SharedMultiplexerSocket,
    config: Arc<ClientConfig>,
    cluster_config: StreamfyClusterConfig,
    versions: Versions,
...
impl Streamfy {
    /// Creates a new Streamfy client using the current profile from `~/.streamfy/config`
    ///
    /// If there is no current profile or the `~/.streamfy/config` file does not exist,
    ...
    ///
    /// ```no_run
    /// # use streamfy::{Streamfy, StreamfyError};
    ...
    pub async fn connect() -> Result<Self> {
        let cluster_config = StreamfyClusterConfig::load()?;
        Self::connect_with_config(&cluster_config).await
    }

    /// Creates a new Streamfy client with the given configuration
    ///
    /// # Example
    ///
    /// ```no_run
    /// # use streamfy::{Streamfy, StreamfyError, StreamfyClusterConfig};
    ...
    /// let streamfy = Streamfy::connect_with_config(&config).await?;
    /// # Ok(())
    /// # }
    /// ```
    pub async fn connect_with_config(config: &StreamfyClusterConfig) -> Result<Self> {
        ...
        info!(
            streamfy_crate_version = env!("CARGO_PKG_VERSION"),
            "Connecting to Streamfy cluster"
        );
        Self::connect_with_connector(connector, config).await
    }

    /// Creates a new Streamfy client with the given profile
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    /// let streamfy = Streamfy::connect_with_profile("local").await?;
    /// # Ok(())
    /// # }
    /// ```
    pub async fn connect_with_profile(profile: &str) -> Result<Self> {
        let config = StreamfyClusterConfig::load_with_profile(profile)?.context(format!(
            "Failed to load cluster config with profile `{profile}`"
        ))?;
        Self::connect_with_config(&config).await
    }

    /// Creates a new Streamfy client with the given connector and configuration
    pub async fn connect_with_connector(
        ...
        //Self::connect_with_client_config(client_config, streamfy_config).await
    ...
    ///
    /// Currently, producers are scoped to a specific Streamfy topic.
    ...
    ///
    /// ```no_run
    /// # use streamfy::{Streamfy, StreamfyError, RecordKey};
    /// # async fn do_produce_to_topic(streamfy: &Streamfy) -> anyhow::Result<()> {
    /// let producer = streamfy.topic_producer("my-topic").await?;
    /// producer.send(RecordKey::NULL, "Hello, Streamfy!").await?;
    /// # Ok(())
    /// # }
    /// ```
    ...
    ///
    /// ```no_run
    /// # use streamfy::{Streamfy, StreamfyError, RecordKey, TopicProducerConfigBuilder};
    /// # async fn do_produce_to_topic(streamfy: &Streamfy) -> anyhow::Result<()> {
    /// let config = TopicProducerConfigBuilder::default().batch_size(500).build()?;
    /// let producer = streamfy.topic_producer_with_config("my-topic", config).await?;
    /// producer.send(RecordKey::NULL, "Hello, Streamfy!").await?;
    /// # Ok(())
    /// # }
    /// ```
    pub async fn topic_producer_with_config(
        ...
        if !spu_pool.topic_exists(topic.clone()).await? {
            return Err(StreamfyError::TopicNotFound(topic).into());
        }
    ...
    ///
    /// Currently, consumers are scoped to both a specific Streamfy topic
    ...
    ///
    /// ```no_run
    /// # use streamfy::{Streamfy, Offset, StreamfyError, PartitionSelectionStrategy};
    /// # async fn do_consume_from_partitions(streamfy: &Streamfy) -> anyhow::Result<()> {
    /// # let consumer = streamfy.consumer(PartitionSelectionStrategy::All("my-topic".to_string())).await?;
    ...
    /// to store consumed offsets in the Streamfy cluster, and use it later to continue reading from
    /// the last seen record.
    ///
    /// If the `offset_consumer` property of `ConsumerConfigExt` is specified, the Streamfy fetches
    /// the offset by id and starts the stream from the next available record. If the offset does not exist, the Streamfy creates it.
    ...
    ///
    /// The Streamfy saves offsets once one called [`ConsumerStream::offset_commit()`] method followed by [`ConsumerStream::offset_flush()`].
    ...
    /// async fn do_consume_with_manual_commits(streamfy: &Streamfy) -> anyhow::Result<()> {
    ...
    /// };
    /// use futures_util::StreamExt;
    /// async fn do_consume_with_auto_commits(streamfy: &Streamfy) -> anyhow::Result<()> {
    ///    let mut stream = streamfy
    ///        .consumer_with_config(
    ///            ConsumerConfigExtBuilder::default()
    ///                .topic("my-topic".to_string())
    ...
    pub async fn consumer_with_config(
        ...
        impl ConsumerStream<Item = std::result::Result<Record, streamfy_protocol::link::ErrorCode>>
        + use<>,
    > {
    ...
    > {
        ...
        let topic_spec = topics
            .lookup_by_key(topic)
            .await?
            .ok_or_else(|| StreamfyError::TopicNotFound(topic.to_string()))?
            .spec;
    ...
    pub async fn consumer_offsets(&self) -> Result<Vec<ConsumerOffset>> {
        ...
        let response = socket
            .send_receive(
                streamfy_spu_schema::server::consumer_offset::FetchConsumerOffsetsRequest {
                    ..Default::default()
                },
            )
            .await?;
    ...
    pub async fn delete_consumer_offset(
        ...
        replica_id: impl Into<streamfy_protocol::record::ReplicaKey>,
    ) -> Result<()> {
        use streamfy_protocol::link::ErrorCode;
        ...
        let response = socket
            .send_receive(
                streamfy_spu_schema::server::consumer_offset::DeleteConsumerOffsetRequest {
    ...
    /// Provides an interface for managing a Streamfy cluster
    ///
    /// # Example
    ///
    /// ```no_run
    /// # use streamfy::{Streamfy, StreamfyError};
    /// # async fn do_get_admin(streamfy: &mut Streamfy) -> anyhow::Result<()> {
    /// let admin = streamfy.admin().await;
    /// # Ok(())
    /// # }
    /// ```
    pub async fn admin(&self) -> StreamfyAdmin {
        let socket = self.create_serial_client();
        let metadata = self.metadata.clone();
        StreamfyAdmin::new(socket, metadata)
    }
...
fn check_platform_compatible(cluster_version: &Version) -> Result<(), StreamfyError> {
    ...
    if *cluster_version < client_minimum_version {
        return Err(StreamfyError::MinimumPlatformVersion {
...
mod wasm_tests {
    ...
    pub struct StreamfyWebsocketConnector {}
    impl StreamfyWebsocketConnector {
        pub fn new() -> Self {
            Self {}
        }
    }
    #[async_trait(?Send)]
    impl TcpDomainConnector for StreamfyWebsocketConnector {
        async fn connect(
            &self,
            addr: &str,
    ...
    #[wasm_bindgen_test]
    async fn my_test() {
        let config = StreamfyClusterConfig::new("ws://localhost:3000");
        let client =
            Streamfy::connect_with_connector(Box::new(StreamfyWebsocketConnector::new()), &config)
                .await;
        assert!(client.is_ok());
