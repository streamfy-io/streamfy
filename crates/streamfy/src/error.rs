...
use streamfy_protocol::link::smartmodule::SmartModuleTransformRuntimeError;
use streamfy_compression::CompressionError;
use streamfy_socket::SocketError;
use streamfy_sc_schema::ApiError;

use crate::config::ConfigError;
use crate::producer::ProducerError;
use crate::producer::TopicProducerConfigBuilderError;

pub type Result<T, Err = StreamfyError> = std::result::Result<T, Err>;

/// Possible errors that may arise when using Streamfy
#[derive(thiserror::Error, Debug)]
pub enum StreamfyError {
    #[error(transparent)]
    Io(#[from] IoError),
...
To interact with this cluster, please install the matching CLI version using the following command:
    curl -fsS https://raw.githubusercontent.com/streamfy-io/streamfy/master/install.sh | VERSION={cluster_version} bash
    ")]
...
To interact with this cluster, please install the matching CLI version using the following command:
    ...
    SmartModuleConfigBuilder(#[from] streamfy_smartengine::SmartModuleConfigBuilderError),
...
pub fn anyhow_version_error(platform_ver: &str) -> anyhow::Error {
    ...
    anyhow!(
        "Streamfy Client {client_ver} and Cluster {platform_ver} versions are not compatible. Please use a client compatible with {platform_ver}"
    )
}
