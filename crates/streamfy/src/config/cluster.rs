...
use crate::{config::TlsPolicy, StreamfyError};

use super::ConfigFile;

//NOTE: this is to avoid breaking changes as we rename it to StreamfyClusterConfig
/// Streamfy client configuration
pub type StreamfyConfig = StreamfyClusterConfig;

/// Streamfy Cluster Target Configuration
/// This is part of profile
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[non_exhaustive]
pub struct StreamfyClusterConfig {
    /// The address to connect to the Streamfy cluster
...
impl StreamfyClusterConfig {
    /// get current cluster config from default profile
    pub fn load() -> Result<Self, StreamfyError> {
    ...
    pub fn load_with_profile(profile_name: &str) -> Result<Option<Self>, StreamfyError> {
        let config_file = ConfigFile::load_default_or_new()?;
...
impl TryFrom<StreamfyClusterConfig> for streamfy_socket::ClientConfig {
    type Error = anyhow::Error;
    fn try_from(config: StreamfyClusterConfig) -> Result<Self, Self::Error> {
        let connector = streamfy_future::net::DomainConnector::try_from(config.tls.clone())?;
        Ok(Self::new(
            &config.endpoint,
            connector,
