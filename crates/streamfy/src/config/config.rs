...
use streamfy_types::config_file::LoadConfigError;
use thiserror::Error;
...
use crate::{StreamfyClusterConfig, StreamfyError};
...
pub enum ConfigError {
    ...
    #[error("Failed to deserialize Streamfy config {msg}")]
    TomlError {
        msg: String,
...
impl ConfigFile {
    ...
    pub fn load(optional_path: Option<String>) -> Result<Self, StreamfyError> {
    ...
    /// read from file
    fn from_file<T: AsRef<Path>>(path: T) -> Result<Self, StreamfyError> {
    ...
    /// 3) home directory ~/.streamfy/config
    ...
    /// Save to file
    pub fn save(&self) -> Result<(), StreamfyError> {
    ...
    pub fn add_or_replace_profile(
        ...
        match config.cluster_mut(profile_name) {
            ...
            None => {
                let mut new_cluster = StreamfyClusterConfig::new(cluster_addr);
...
pub struct Config {
    ...
    pub cluster: HashMap<String, StreamfyClusterConfig>,
...
impl Config {
    ...
    pub fn new_with_local_cluster(domain: String) -> Self {
        let cluster = StreamfyClusterConfig::new(domain);
    ...
    /// add new cluster
    pub fn add_cluster(&mut self, cluster: StreamfyClusterConfig, name: String) {
    ...
    /// # use streamfy::config::{Config, Profile};
    /// let mut config = Config::new();
    /// let cluster = StreamfyClusterConfig::new("https://cloud.streamfy.io".to_string());
    /// config.add_cluster(cluster, "streamfy-cloud".to_string());
    /// let profile = Profile::new("streamfy-cloud".to_string());
    /// config.add_profile(profile, "streamfy-cloud".to_string());
    ///
    /// config.delete_cluster("streamfy-cloud").unwrap();
    /// assert!(config.cluster("streamfy-cloud").is_none());
    /// ```
    pub fn delete_cluster(&mut self, cluster_name: &str) -> Option<StreamfyClusterConfig> {
    ...
    /// config.add_profile(profile, "streamfy-cloud".to_string());
    ///
    /// let conflicts = config.delete_cluster_check("streamfy-cloud").unwrap_err();
    /// assert_eq!(conflicts, vec!["streamfy-cloud"]);
    /// ```
    ...
    pub fn current_profile(&self) -> Result<&Profile, StreamfyError> {
        let profile = self
            .current_profile
            .as_ref()
    ...
    /// Returns the StreamfyClusterConfig belonging to the current profile.
    pub fn current_cluster(&self) -> Result<&StreamfyClusterConfig, StreamfyError> {
    ...
    /// Returns the mutable reference to StreamfyClusterConfig belonging to the current profile.
    pub fn current_cluster_mut(&mut self) -> Result<&mut StreamfyClusterConfig, StreamfyError> {
    ...
    /// Returns the StreamfyClusterConfig belonging to the named profile.
    pub fn cluster_with_profile(&self, profile_name: &str) -> Option<&StreamfyClusterConfig> {
        self.profile
            .get(profile_name)
            .and_then(|profile| self.cluster.get(&profile.cluster))
    }

    /// Returns a reference to the named StreamfyClusterConfig.
    pub fn cluster(&self, cluster_name: &str) -> Option<&StreamfyClusterConfig> {
        self.cluster.get(cluster_name)
    }

    /// Returns a mutable reference to the named StreamfyClusterConfig.
    pub fn cluster_mut(&mut self, cluster_name: &str) -> Option<&mut StreamfyClusterConfig> {
