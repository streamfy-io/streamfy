//! The Streamfy Producer module allows applications to send messages to topics in the Streamfy cluster.
//!
//! # Overview
//!
//! This module provides the necessary structures and functions to produce messages to a Streamfy topic.
...
use streamfy_compression::Compression;
#[cfg(feature = "compress")]
use streamfy_sc_schema::topic::CompressionAlgorithm;
use streamfy_sc_schema::topic::TopicSpec;
use streamfy_sc_schema::partition::PartitionSpec;
...
pub use streamfy_protocol::record::{RecordKey, RecordData};
    ...
    S: SpuPool + Send + Sync + 'static,
{
    ...
    sm_chain: Option<Arc<RwLock<streamfy_smartengine::SmartModuleChainInstance>>>,
    #[allow(unused)]
    ...
    if #[cfg(feature = "smartengine")] {
                ...
                S: SpuPool + Send + Sync + 'static,
        {
            ...
            pub async fn with_chain(mut self, chain_builder: SmartModuleChainBuilder) -> Result<Self> {
                let mut chain_instance = chain_builder.initialize(&SM_ENGINE).map_err(|e| StreamfyError::Other(format!("SmartEngine - {e:?}")))?;
    ...
    S: SpuPool + Send + Sync + 'static,
{
    ...
    ///
    /// ```
    /// # use streamfy::{TopicProducerPool, StreamfyError};
    ...
    pub async fn send(
        ...
        cfg_if::cfg_if! {
            if #[cfg(feature = "smartengine")] {
                ...
                use streamfy_smartengine::DEFAULT_SMARTENGINE_VERSION;
                use streamfy_smartmodule::dataplane::smartmodule::SmartModuleInput;
                ...
                if let Some(
                    ...
                    let output = sm_chain.process(sm_input).map_err(|e| StreamfyError::Other(format!("SmartEngine - {e:?}")))?;
...
fn determine_producer_compression_algo(
    config: Arc<TopicProducerConfig>,
    topic_spec: streamfy_sc_schema::topic::TopicSpec,
) -> Result<Compression> {
    let result = match topic_spec.get_compression_type() {
        ...
        CompressionAlgorithm::Gzip => match config.compression {
            ...
            Some(compression_config) => return Err(StreamfyError::Producer(ProducerError::InvalidConfiguration(
...
mod tests {
    ...
    use streamfy_future::timer::sleep;
    use streamfy_protocol::record::{RecordKey, ReplicaKey};
    use streamfy_sc_schema::{
        partition::{PartitionSpec},
        store::MetadataStoreObject,
        topic::TopicSpec,
    };
    use streamfy_socket::{ClientConfig, SocketError, StreamSocket, VersionedSerialSocket};
    use streamfy_stream_dispatcher::metadata::local::LocalMetadataItem;
    use streamfy_types::{PartitionId, SpuId};

    use crate::{
        metrics::ClientMetrics,
        spu::SpuPool,
        sync::{MetadataStores, StoreContext},
        StreamfyError, TopicProducer, TopicProducerConfig,
    };
    ...
    impl SpuPool for SpuPoolMock {
        ...
        async fn connect_to_leader(&self, _leader: SpuId) -> Result<StreamSocket, StreamfyError> {
            todo!()
        }
        ...
        async fn create_serial_socket_from_leader(
            &self,
            _leader_id: SpuId,
        ) -> Result<VersionedSerialSocket, StreamfyError> {
            todo!()
        }

        async fn topic_exists(&self, _topic: String) -> Result<bool, StreamfyError> {
            todo!()
        }
