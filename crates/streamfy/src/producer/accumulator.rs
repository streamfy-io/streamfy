...
use streamfy_future::future::timeout;
...
use streamfy_spu_schema::produce::ProduceResponse;
use streamfy_protocol::record::Record;
use streamfy_socket::SocketError;
use streamfy_types::{PartitionId, Timestamp, PartitionCount};
...
mod test {
    use super::*;
    use streamfy_protocol::record::{Record, RawRecords};
    use streamfy_spu_schema::produce::{PartitionProduceResponse, TopicProduceResponse};
    ...
    async fn test_record_accumulator() {
        ...
        accumulator
            ...
            .expect("failed push");
        assert!(
            streamfy_future::future::timeout(timeout, batches.listen_new_batch())
                .await
                .is_ok()
        );

        assert!(
            streamfy_future::future::timeout(timeout, batches.listen_batch_full())
                .await
                .is_err()
        );
        accumulator
