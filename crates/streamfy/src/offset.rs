...
use streamfy_spu_schema::server::fetch_offset::FetchOffsetsRequest;
use streamfy_spu_schema::server::fetch_offset::FetchOffsetPartitionResponse;
...
/// Describes the location of an event stored in a Streamfy partition
///
/// All Streamfy events are stored as a log inside a partition. A log
...
/// changes.
///
/// Streamfy allows you to set a retention policy that determines how
/// long your events should live. Once an event has outlived your
/// retention policy, Streamfy may delete it to save resources. Whenever
...
impl Offset {
    ...
    /// assert!(Offset::absolute(-10).is_err());
    /// ```
    pub fn absolute(index: i64) -> Result<Offset, StreamfyError> {
        if index < 0 {
            return Err(StreamfyError::NegativeOffset(index));
        }
        Ok(Self {
    ...
    /// offset. In order to save space, Streamfy may sometimes delete events
...
pub(crate) async fn fetch_offsets(
    ...
    replica: &ReplicaKey,
) -> Result<FetchOffsetPartitionResponse, StreamfyError> {
