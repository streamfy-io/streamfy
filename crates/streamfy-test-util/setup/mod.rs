...
mod cluster {
    ...
    impl TestCluster {
        ...
        pub async fn start(&mut self) -> Result<Streamfy> {
            ...
            let streamfy_config = if self.option.tls {
                let (client, _server) = load_tls(&self.option.tls_user);
                StreamfyClusterConfig::new(cluster_status.address()).with_tls(client)
            } else {
                StreamfyClusterConfig::new(cluster_status.address())
            };
