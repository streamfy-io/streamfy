...
impl LocalEnvDriver {
    ...
    fn load_config(option: &EnvironmentSetup) -> LocalConfig {
        ...
        // Make sure to use the test build of 'streamfy' for cluster components
        let test_exe = std::env::current_exe();
        let build_dir = test_exe
            .ok()
            .and_then(|it| it.parent().map(|it| it.join("streamfy")));
...
impl EnvironmentDriver for LocalEnvDriver {
    ...
    fn create_cluster_manager(&self) -> Box<dyn streamfy_cluster::runtime::spu::SpuClusterManager> {
