//use streamfy::consumer::{PartitionSelectionStrategy, ConsumerConfig};

use tracing::debug;
use anyhow::Result;

use streamfy::consumer::{BoxConsumerStream, ConsumerConfigExt, ConsumerConfigExtBuilder};
use streamfy::{TopicProducerPool, Streamfy, Offset, RecordKey};
use streamfy::metadata::topic::TopicSpec;
use streamfy::TopicProducerConfig;
...
impl TestDriver {
    ...
    pub fn client(&self) -> &Streamfy {
        self.client
            .as_ref()
            .expect("Not connected to Streamfy cluster")
    }
    ...
    pub async fn create_producer_with_config(
        ...
        let streamfy_client = self.create_client().await.expect("cant' create client");
        match streamfy_client
    ...
    pub async fn get_consumer_with_config(&self, config: ConsumerConfigExt) -> BoxConsumerStream {
        let streamfy_client = self.create_client().await.expect("cant' create client");
        match streamfy_client.consumer_with_config(config).await {
            Ok(client) => {
                //self.consumer_num += 1;
                Box::pin(client)
            }
    ...
    /// create new streamfy client
    async fn create_client(&self) -> Result<Streamfy> {
        Streamfy::connect().await
    }
}
