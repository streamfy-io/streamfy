...
pub enum ChannelConfigError {
    ...
    #[error("Failed to deserialize Streamfy config: {0}")]
    TomlError(#[from] toml::de::Error),
}
#[derive(Debug, Clone, Eq, PartialEq, Serialize, Deserialize)]
pub struct StreamfyChannelConfig {
    pub path: PathBuf,
    pub config: ChannelConfig,
}

impl Default for StreamfyChannelConfig {
    fn default() -> Self {
        Self {
...
impl StreamfyChannelConfig {
    ...
    pub fn from_file<T: AsRef<Path>>(path: T) -> Result<Self, ChannelConfigError> {
        ...
        let config = StreamfyChannelConfig {
    ...
    pub fn get_channel(&self, channel_name: &str) -> Option<StreamfyChannelInfo> {
        self.config
            .channel
            .get(channel_name)
    ...
    pub fn remove_channel(&mut self, channel_name: String) -> Result<Option<StreamfyChannelInfo>> {
...
pub struct ChannelConfig {
    ...
    pub channel: HashMap<String, StreamfyChannelInfo>,
}
...
impl ChannelConfig {
    ...
    pub fn channel(&self) -> HashMap<String, StreamfyChannelInfo> {
        self.channel.clone()
    }
}
...
#[derive(Debug, Clone, Default, Eq, PartialEq, Serialize, Deserialize)]
pub struct StreamfyChannelInfo {
...
impl StreamfyChannelInfo {
    ...
    pub fn new_channel(channel_name: &str, image_tag_strategy: ImageTagStrategy) -> Self {
        ...
        cfg_if! {
            if #[cfg(not(target_os = "windows"))] {
                let streamfy_bin_name = format!("streamfy-{channel_name}");
            } else {
                let streamfy_bin_name = format!("streamfy-{channel_name}.exe");
            }
        }
    ...
    pub fn dev_channel() -> Self {
        cfg_if! {
            if #[cfg(not(target_os = "windows"))] {
                let streamfy_bin_name = "streamfy".to_string();
            } else {
                let streamfy_bin_name = "streamfy.exe".to_string();
            }
        }
        // streamfy binary is expected to be in same dir as the current binary
        if let Ok(mut exe) = std::env::current_exe() {
            exe.set_file_name(streamfy_bin_name);
            ...
            Self {
                binary_location: PathBuf::from(streamfy_bin_name),
                extensions: PathBuf::from("extensions"),
                image_tag_strategy: ImageTagStrategy::Git,
            }
        }
...
// Check if we're running Streamfy in the location our installer places binaries
// This is used to decide whether to use channel config
pub fn is_streamfy_bin_in_std_dir(streamfy_bin: &Path) -> bool {
    let streamfy_home_dir = if let Some(mut streamfy_home) = home_dir() {
        streamfy_home.push(CLI_CONFIG_PATH);
        streamfy_home
    } else {
        return false;
    };

    // Verify if streamfy_bin is in the same directory as {home_dir/CLI_CONFIG_PATH}
    streamfy_bin.starts_with(streamfy_home_dir)
}
