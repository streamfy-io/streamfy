...
use streamfy::{RecordKey, TopicProducerPool};
use streamfy_connector_common::{
    connector,
    Result
};
...
async fn start(config: CustomConfig, producer: TopicProducerPool) -> Result<()> {
    ...
    for i in 1..1000 {
        let value = format!("Hello, Streamfy - {i}");
...
use streamfy_connector_common::{connector, consumer::ConsumerStream, Result};
