...
The key operation of the API would be to send consumed offsets associated with a streamfy topic and id for offset tracking.
...
```rust
use streamfy::consumer::ConsumerConfigBuilder;
use streamfy::consumer::OffsetManagement;

async fn offset_example() -> Result<()> {
    let client = streamfy::connect();
...
```
$ streamfy topic create my-topic1 --partition 2
$ streamfy consume my-topic1 --consumer c1 --partition 0
$ streamfy consume my-topic1 --consumer c1 --partition 1

$ streamfy topic create my-topic2 --partition 3
$ streamfy consume my-topic2 --consumer c2 -all

$ streamfy topic create my-topic3
$ streamfy consume my-topic3 -c c3                  <--- creates a consumer at partition 0
```

List consumers:

```
$ streamfy consumer list
...
```
$ streamfy consumer delete c1 --partition 0
consumer "c1" on partition "0" deleted
```

Without partition:
```
$ streamfy consumer delete c1
consumer "c1" on partition "0" deleted
consumer "c1" on partition "1" deleted
```


`streamfy topic describe TOPIC` should be modified to output consumer settings

```
$ streamfy topic describe my-topic1
   Name:my-topic1
   Type:computed
   Partition Count:2
...
```
$ streamfy topic delete my-topic1
...
SSDK has a more generalized offset management for managing its offset for state consumption. Implementation is available from that codebase for reference, but the generalization is recommended to be simplified for an in-streamfy implementation.
...
For a simpler streamfy interface, it is recommended to keep a simpler offset interface vs specific consumer ids, and deferring consumer group balancing (and offset management) as a separate feature.

In particular for a streamfy connector consume of an offset record would likely only after some external sink has returned with confirmation that data was committed. For a file system a confirmation might be a flush completion, or a database, a successful insert or upsert, S3 an object commit, etc. Only after the confirmation would a consumer want to commit an offset.
