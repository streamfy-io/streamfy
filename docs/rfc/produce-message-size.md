# Enhancing Streamfy Message Sizes

This RFC proposes modifications to Streamfy's handling of message sizes. 
...
If a single record exceeds the defined `batch_size`, Streamfy will process the record in a new batch without the `batch_size` as limit, ensuring that larger messages are not discarded or delayed. If the record does not exceed the `batch_size`, Streamfy will process the record as part of an already existing batch or create a new one if the batch is full.

2. Handling Larger Messages than the Max Request Size

Streamfy will have a new configuration parameter, max_request_size, that will define the maximum size of a request that can be sent by the producer. This configuration will make Streamfy display errors when a message exceeds the defined `max_request_size`, even if it's a message with only one record or a batch of records.

3. Compression Behavior

Streamfy will ensure that configuration limits that use size constraints, such as `batch_size` and `max_request_size` will only use the uncompressed message size.
...
```bash
streamfy topic create large-data-topic
printf 'This is a sample line. ' | awk -v b=500000 '{while(length($0) < b) $0 = $0 $0}1' | cut -c1-500000 > large-data-file.txt
```

### Batch Size

`batch_size` will define the maximum size of a batch of records that can be sent by the producer. If a record exceeds this size, Streamfy will process the record in a new batch without the `batch_size` as limit.

```bash
streamfy produce large-data-topic --batch-size 16536 --file large-data-file.txt --raw
```
...
`max_request_size` will define the maximum size of a message that can be sent by the producer. If a message exceeds this size, Streamfy will throw an error. Even if it's a message with only one record or a batch of them.

```bash
streamfy produce large-data-topic --max-request-size 16384 --file large-data-file.txt --raw
```
...
```bash
streamfy produce large-data-topic --batch-size 16536 --compression gzip --file large-data-file.txt --raw
streamfy produce large-data-topic --max-request-size 16384 --compression gzip --file large-data-file.txt --raw
```
