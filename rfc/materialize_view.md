Streamfy & Streamfy Cloud users need the ability to perform operations on a time-bound window and generate a materialized view.

## Motivation 

Today, Streamfy supports record-by-record processing with the ability to apply transformation one record at a time. When a multi-record stream-based operation is required, Streamfy users create a Microservice that reads the records, applies an operation, and returns the result to a new stream. Unfortunately, these Microservices are managed independently of Streamfy, significantly increasing the complexity of building simple Real-Time Apps. 

This PRD is a proposal to add the ability to compute aggregates inside Streamfy. This solution should eliminate the need to deploy and operate separate Microservices to perform stream-based computations. 

In a larger context, time-based computations bring Streamfy closer to Flink and Spark, where our users won’t need to run multiple stacks to perform sums, averages, anomaly detections, etc.


## Requirements

Streamfy's data streaming layer (aka topic/partitions) will continue operating as before. The stream processing component is an additional layer that runs on top. This stream-processing engine is defined as a separate object, as described below.
...
Streamfy operates on binary records, where the data interpretation is opaque to the system. However, with stream-based computation, the system must understand the data it operates on.

Defining a materialized view in Streamfy requires the following steps:
...
```
$ streamfy topic create metrics --columns metrics_columns.yaml
```

Use `metrics.json` file we defined above to load events into the topic:

```
$ streamfy produce --json-file metrics.json metrics
parsed successfully
```
...
```
$ streamfy consume metrics -Bd --output table
...
```
$ streamfy view create usage usage-view.yaml
view created
```
...
```
$ streamfy topic create pricing --columns pricing_columns.yaml
```

Add pricing data from `JSON` file defined above to the topic:

```
 $ streamfy produce --json-file pricing.json pricing
parsed successfully
```
...
```
$ streamfy consume pricing -Bd --output table
...
```
$ streamfy view create pricing pricing-view.yaml
view created
```
...
```
$ streamfy view consume pricing -d
...
```
$ streamfy view eval pricing `getPrice("network", "2023-02-18 06:41:48")`
0.06
```
...
```
$ streamfy view create usage-pricing usage-pricing-view.yaml
view created
```
