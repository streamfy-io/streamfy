# Streamfy Test Runner

Used to run a various test against `Streamfy` platform.

This assumes you have read Streamfy doc and setup or access to a Streamfy cluster .

## Setting up Test runer

* Build all `Streamfy` crates
* Build development kubernetes image

Must run test runner from streamfy repo root (issue: #[860](https://github.com/streamfy/streamfy/issues/860))
...
## Testing Streamfy with different cargo compiler profiles
...
Optional: Set up alias to run development version of Streamfy and Test runner CLI.

```
cargo build
alias streamfy-test=./target/debug/streamfy-test
```

### Release profile

It's important to test Streamfy's release artifact (without the debug symbols) in order to reproduce the same behavior as in production.

This is as easy as adding `--release` to the `cargo build` command.

Optional: Set up alias to run release version of Streamfy and Test runner CLI.

```
cargo build --release
alias streamfy-test=./target/release/streamfy-test
```

# Running Test runner

```
streamfy-test <test-name> [FLAGS] [OPTIONS] -- [SUBCOMMAND OPTIONS]
```

> Test runner testing doesn't work when VERSION set to an unpublished version. Workaround: Run `make build_k8_image` and use `--develop` flag with `streamfy-test` (issue #[859](https://github.com/streamfy/streamfy/issues/859))
...
> Expected behavior of local clusters that streamfy-test start:
>
...
`streamfy-test smoke [environment vars] -- [smoke test vars]`
...
```
$ streamfy-test smoke --cluster-start --cluster-delete
Start running streamfy test runner
Starting cluster and testing connection
remove cluster skipped
installing cluster
🛠️  Installing Streamfy
     ✅ Streamfy app chart has been installed
...
```
$ streamfy-test smoke --local -- --producer-iteration 10
```

### Run test using your current Streamfy cluster

```
$ streamfy-test smoke --local -- --producer-iteration 10
$ streamfy-test smoke -- --producer-iteration 200
```
...
```
$ streamfy-test smoke -- --producer-iteration=400 --consumer-wait
```

## Anatomy of a new test

There are 2 parts to adding new tests.
1. Creating a new test in the `tests` module + annotating with `#[streamfy_test]`
...
Write new tests in the `tests` module of the `streamfy-test` crate. 
...
#[clap(name = "Streamfy Test Example Test")]
...
#[streamfy_test()]
pub fn example(mut test_driver: StreamfyTestDriver, mut test_case: TestCase) {
...
### Using the streamfy test macro
The `#[streamfy_test]` macro will expand your test to create a topic and check if the `TestCase` meets your test requirements.
...
```rust
#[streamfy_test(min_spu=2, topic="test_topic")]
pub fn run(client: Arc<Streamfy>, mut option: TestCase) {
    println!("Hello world!")
}
```
...
```rust
#[streamfy_test(min_spu=2, topic="test_topic", async)]
pub async fn run(client: Arc<Streamfy>, mut option: TestCase) {
    println!("Hello world!")
}
```
