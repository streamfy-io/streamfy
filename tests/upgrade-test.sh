...
function cleanup() {
 ...
 #   $STREAMFY_BIN cluster delete || true
 #   $STREAMFY_BIN cluster delete --sys || true
}
...
function validate_cluster_stable() {
	...
	$STABLE_STREAMFY cluster start --local
    else
	$STABLE_STREAMFY cluster start --k8
    fi
    ...
    $STABLE_STREAMFY version
    validate_cli_version $STABLE_STREAMFY $STABLE_VERSION
    validate_platform_version $STABLE_STREAMFY $STABLE_VERSION
    ci_check;


    echo "Create test topic: ${STABLE_TOPIC}"
    $STABLE_STREAMFY topic create ${STABLE_TOPIC} 
    # $STABLE_STREAMFY topic create ${STABLE_TOPIC}-delete 
    ci_check;

    echo "Create mirror"
    $STABLE_STREAMFY remote register stable-remote

    # Validate consume on topic before produce
    # https://github.com/streamfy/streamfy/issues/1819
    echo "Validate consume on \"${STABLE_TOPIC}\" before producing"
    $STABLE_STREAMFY consume -B -d ${STABLE_TOPIC} 2>/dev/null
    ci_check;

    echo "Producing test data to ${STABLE_TOPIC}"
    cat data1.txt.tmp | $STABLE_STREAMFY produce ${STABLE_TOPIC}
    # cat data1.txt.tmp | $STABLE_STREAMFY produce ${STABLE_TOPIC}-delete
    ci_check;

    echo "Validate test data w/ v${STABLE} CLI matches expected data created BEFORE upgrading cluster + CLI to v${PRERELEASE}"
    $STABLE_STREAMFY consume -B -d ${STABLE_TOPIC} 2>/dev/null | tee output.txt.tmp
    ci_check;
    ...
    # $STABLE_STREAMFY topic delete ${STABLE_TOPIC}-delete 
    echo "stable validated"


    $STABLE_STREAMFY partition list
    ci_check;
...
function validate_upgrade_cluster_to_prerelease() {
    ...
    local STREAMFY_BIN_ABS_PATH=$(${READLINK} -f $STREAMFY_BIN)
    ...
    if [[ ! -z "$USE_LATEST" ]];
    then
        # Use the "latest" streamfy channel
        ...
        STREAMFY_IMAGE_TAG_STRATEGY=version-git \
        $STREAMFY_BIN_ABS_PATH cluster upgrade --force
        ...
        echo "Using Streamfy binary located @ ${STREAMFY_BIN_ABS_PATH}"
        $STREAMFY_BIN_ABS_PATH cluster upgrade --force --develop
    fi
    if [[ "$STREAMFY_MODE" == "local" ]]; then
	echo "Resuming local cluster"
	$STREAMFY_BIN_ABS_PATH cluster resume
    fi
    popd
    ...
    $STREAMFY_BIN_ABS_PATH version
    validate_cli_version $STREAMFY_BIN_ABS_PATH $TARGET_VERSION
    ci_check;

    validate_platform_version $STREAMFY_BIN_ABS_PATH $TARGET_VERSION
    ci_check;

    echo "Create test topic: ${PRERELEASE_TOPIC}"
    $STREAMFY_BIN_ABS_PATH topic create ${PRERELEASE_TOPIC}

    # Validate consume on topic before produce
    # https://github.com/streamfy/streamfy/issues/1819
    echo "Validate consume on \"${PRERELEASE_TOPIC}\" before producing"
    $STREAMFY_BIN_ABS_PATH consume -B -d ${PRERELEASE_TOPIC} 2>/dev/null

    #kubectl get pods
    #kubectl get spu

    echo "Producing test data to ${PRERELEASE_TOPIC}"
    cat data2.txt.tmp | $STREAMFY_BIN_ABS_PATH produce ${PRERELEASE_TOPIC}


    echo "Validate test data w/ v${TARGET_VERSION} CLI matches expected data AFTER upgrading cluster + CLI to v${TARGET_VERSION}"
    $STREAMFY_BIN_ABS_PATH consume -B -d ${PRERELEASE_TOPIC} 2>/dev/null | tee output.txt.tmp
    ...
    # https://github.com/streamfy/streamfy/issues/1819
    echo "Validate consume on \"${STABLE_TOPIC}\" before producing"
    $STREAMFY_BIN_ABS_PATH consume -B -d ${STABLE_TOPIC} 2>/dev/null

    # Exercise older topics
    echo "Producing test data to ${STABLE_TOPIC}"
    cat data2.txt.tmp | $STREAMFY_BIN_ABS_PATH produce ${STABLE_TOPIC}
    ...
    $STREAMFY_BIN_ABS_PATH consume -B -d ${STABLE_TOPIC} | tee output.txt.tmp
    ...
    #echo "Validate deleting topic created by v${STABLE} CLI"
    # $STREAMFY_BIN_ABS_PATH topic delete ${STABLE_TOPIC} 

    # echo "Validate deleting topic created by v${TARGET_VERSION} CLI"
    #$STREAMFY_BIN_ABS_PATH topic delete ${PRERELEASE_TOPIC} 
}
...
function validate_cli_version() {
    ...
    # Get the streamfy version output
    # Parse out CLI version
    CLI_VERSION=$($CUR_STREAMFY_BIN version | grep "Streamfy CLI" | head -1 | awk '{print $4}')

    if [ "$CLI_VERSION" = "$STREAMFY_SEMVER" ]; then
      echo "✅ CLI Version verified: $CLI_VERSION";
    else
      echo "❌ CLI Version check failed";
      echo "Version reported by streamfy version: $CLI_VERSION";
      echo "Expected version : $STREAMFY_SEMVER";
      exit 1;
    fi
...
function validate_platform_version() {
    ...
    PLATFORM_VERSION=$($CUR_STREAMFY_BIN version | grep "Streamfy Platform" | head -1 | awk '{print $4}')

    # Compare Platform == STREAMFY_SEMVER
    if [ "$PLATFORM_VERSION" = "$STREAMFY_SEMVER" ]; then
      echo "✅ Platform Version verified: $PLATFORM_VERSION";
    else
      echo "❌ Platform Version check failed";
      echo "Version reported by streamfy version: $PLATFORM_VERSION";
      echo "Expected version : $STREAMFY_SEMVER";
      exit 1;
    fi
