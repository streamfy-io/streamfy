...
@test "Can create a mirror topic" {
    ...
    run timeout 15s "$STREAMFY_BIN" topic create "$TOPIC_NAME" --mirror-apply remotes.json --home-to-remote
...
@test "Can add a new remote to the mirror topic" {
    run timeout 15s "$STREAMFY_BIN" topic add-mirror "$TOPIC_NAME" "$REMOTE_NAME_2" --home-to-remote
...
@test "Can't add a non existent remote to the mirror topic" {
    run timeout 15s "$STREAMFY_BIN" topic add-mirror "$TOPIC_NAME" "nonexistent-remote" --home-to-remote
...
@test "Can't add a remote to the mirror topic that doesn't exist" {
    run timeout 15s "$STREAMFY_BIN" topic add-mirror "nonexistent-topic" "$REMOTE_NAME" --home-to-remote
...
@test "Can't add a remote to the mirror topic that is already assigned" {
    run timeout 15s "$STREAMFY_BIN" topic add-mirror "$TOPIC_NAME" "$REMOTE_NAME" --home-to-remote
...
@test "List topics" {
    run bash -c 'timeout 15s "$STREAMFY_BIN" topic list | grep "$TOPIC_NAME"'
    assert_success
    assert_line --partial --index 0 "$TOPIC_NAME  to-remote"
}

@test "List partitions" {
    run bash -c 'timeout 15s "$STREAMFY_BIN" partition list | grep "$TOPIC_NAME"'
    assert_success
