...
setup_file() {
    ...
    HOME_PROFILE=$($STREAMFY_BIN profile)
    export HOME_PROFILE
...
@test "Can register first local remote cluster" {
    run timeout 15s "$STREAMFY_BIN" remote register "$REMOTE_NAME"
...
@test "Can register second local remote cluster" {
    run timeout 15s "$STREAMFY_BIN" remote register "$REMOTE_NAME_2"
...
@test "Export home to remote 1" {
    run timeout 15s "$STREAMFY_BIN" remote export "$REMOTE_NAME" --file remote.json
    assert_output ""
    assert_success
...
@test "Export home to remote 2" {
    run timeout 15s "$STREAMFY_BIN" remote export "$REMOTE_NAME_2" --file remote2.json
    assert_output ""
    assert_success
...
@test "Can create a mirror topic with the remote clusters" {
    ...
    run timeout 15s "$STREAMFY_BIN" topic create "$TOPIC_NAME" --mirror-apply remotes_devices.json
...
@test "Can create a mirror topic with the remote clusters reverse" {
    echo "[\"$REMOTE_NAME\",\"$REMOTE_NAME_2\"]" > remotes_devices.json
    run timeout 15s "$STREAMFY_BIN" topic create "$REVERSE_TOPIC_NAME" --mirror-apply remotes_devices.json --home-to-remote
...
@test "Can produce message to reverse mirror topic from home" {
    run bash -c 'echo 3 | timeout 15s "$STREAMFY_BIN" produce "$REVERSE_TOPIC_NAME"'
    assert_success
    run bash -c 'echo c | timeout 15s "$STREAMFY_BIN" produce -p 0 "$REVERSE_TOPIC_NAME"'
    assert_success
    run bash -c 'echo 4 | timeout 15s "$STREAMFY_BIN" produce -m "$REMOTE_NAME" "$REVERSE_TOPIC_NAME"'
    assert_success
    run bash -c 'echo d | timeout 15s "$STREAMFY_BIN" produce -m "$REMOTE_NAME" "$REVERSE_TOPIC_NAME"'
    assert_success
}

@test "Can switch to remote cluster 1" {
    run timeout 15s "$STREAMFY_BIN" profile switch "$REMOTE_PROFILE"
    assert_output ""
    assert_success
}
...
@test "Can't produce message to mirror topic from remote 1 before connect to home" {
    sleep 5
    run bash -c 'echo 1 | timeout 15s "$STREAMFY_BIN" produce "$TOPIC_NAME"'
...
@test "Can connect to the home cluster from remote 1" {
    sleep 2
    run timeout 15s "$STREAMFY_BIN" home connect --file remote.json
...
@test "Home status at remote 1 should show the home cluster connected" {
    sleep 60
    run timeout 15s "$STREAMFY_BIN" home status
...
@test "Can produce message to mirror topic from remote 1" {
    ...
    run bash -c 'echo 1 | timeout 15s "$STREAMFY_BIN" produce "$TOPIC_NAME"'
    assert_success
    run bash -c 'echo a | timeout 15s "$STREAMFY_BIN" produce "$TOPIC_NAME"'
    assert_success
    run bash -c 'echo 2 | timeout 15s "$STREAMFY_BIN" produce "$TOPIC_NAME"'
    assert_success
    run bash -c 'echo b | timeout 15s "$STREAMFY_BIN" produce "$TOPIC_NAME"'
    assert_success
}

@test "Can't produce message to reverse mirror topic from remote 1" {
    run bash -c 'echo 1 | timeout 15s "$STREAMFY_BIN" produce "$REVERSE_TOPIC_NAME"'
...
@test "Can consume message from reverse mirror topic from remote 1" {
    sleep 5
    run timeout 15s "$STREAMFY_BIN" consume "$REVERSE_TOPIC_NAME" -p 0 -B -d
    assert_output 3$'\n'c$'\n'4$'\n'd
    assert_success

    run timeout 15s "$STREAMFY_BIN" consume "$REVERSE_TOPIC_NAME" --mirror "$REMOTE_NAME" -B -d
    assert_output 3$'\n'c$'\n'4$'\n'd
    assert_success
}

@test "Can switch back to home cluster" {
    run timeout 15s "$STREAMFY_BIN" profile switch "$HOME_PROFILE"
    assert_output ""
    assert_success
}
...
@test "Can produce message to reverse mirror topic from home again" {
    run bash -c 'echo 5 | timeout 15s "$STREAMFY_BIN" produce -p 1 "$REVERSE_TOPIC_NAME"'
    assert_success
    run bash -c 'echo e | timeout 15s "$STREAMFY_BIN" produce -p 1 "$REVERSE_TOPIC_NAME"'
    assert_success
    run bash -c 'echo 6 | timeout 15s "$STREAMFY_BIN" produce -m "$REMOTE_NAME_2" "$REVERSE_TOPIC_NAME"'
    assert_success
    run bash -c 'echo f | timeout 15s "$STREAMFY_BIN" produce -m "$REMOTE_NAME_2" "$REVERSE_TOPIC_NAME"'
    assert_success
}

@test "Can switch to remote cluster 2" {
    run timeout 15s "$STREAMFY_BIN" profile switch "$REMOTE_PROFILE_2"
    assert_output ""
    assert_success
}
...
@test "Can connect to the home cluster from remote 2" {
    sleep 2
    run timeout 15s "$STREAMFY_BIN" home connect --file remote2.json
...
@test "Can produce message to mirror topic" {
    sleep 2
    run bash -c 'echo 9 | timeout 15s "$STREAMFY_BIN" produce "$TOPIC_NAME"'
    assert_success
    run bash -c 'echo z | timeout 15s "$STREAMFY_BIN" produce "$TOPIC_NAME"'
    assert_success
    run bash -c 'echo 8 | timeout 15s "$STREAMFY_BIN" produce "$TOPIC_NAME"'
    assert_success
    run bash -c 'echo y | timeout 15s "$STREAMFY_BIN" produce "$TOPIC_NAME"'
    assert_success
}

@test "Can't produce message to reverse mirror topic from remote 2" {
    run bash -c 'echo 9 | timeout 15s "$STREAMFY_BIN" produce "$REVERSE_TOPIC_NAME"'
...
@test "Can consume message from reverse mirror topic from remote 2" {
    ...
    run timeout 15s "$STREAMFY_BIN" consume "$REVERSE_TOPIC_NAME" --mirror "$REMOTE_NAME_2" -B -d
    assert_output 5$'\n'e$'\n'6$'\n'f
    assert_success
}
...
@test "Remote list should show the remote clusters connected" {
    sleep 2
    run timeout 15s "$STREAMFY_BIN" remote list
...
@test "Can consume message from reverse mirror topic from home" {
    ...
    run timeout 15s "$STREAMFY_BIN" consume "$REVERSE_TOPIC_NAME" -p 1 -B -d
    assert_output 5$'\n'e$'\n'6$'\n'f
    assert_success

    run timeout 15s "$STREAMFY_BIN" consume "$REVERSE_TOPIC_NAME" --mirror "$HOME" -B -d
    assert_output 3$'\n'c$'\n'4$'\n'd$'\n'5$'\n'e$'\n'6$'\n'f
    assert_success
}
...
@test "Can consume message from mirror topic produced from remote 1 by partition or remote" {
    sleep 5
    run timeout 15s "$STREAMFY_BIN" consume "$TOPIC_NAME" -p 0 -B -d
    assert_output 1$'\n'a$'\n'2$'\n'b
    assert_success

    run timeout 15s "$STREAMFY_BIN" consume "$TOPIC_NAME" --mirror "$REMOTE_NAME" -B -d
    assert_output 1$'\n'a$'\n'2$'\n'b
    assert_success
}
...
@test "Can consume message from mirror topic produced from remote 2 by partition or remote" {
    sleep 5
    run timeout 15s "$STREAMFY_BIN" consume "$TOPIC_NAME" -p 1 -B -d
    assert_output 9$'\n'z$'\n'8$'\n'y
    assert_success

    run timeout 15s "$STREAMFY_BIN" consume "$TOPIC_NAME" --mirror "$REMOTE_NAME_2" -B -d
    assert_output 9$'\n'z$'\n'8$'\n'y
    assert_success
}
...
@test "Can delete mirror topic" {
    ...
    run timeout 15s "$STREAMFY_BIN" topic delete "$REVERSE_TOPIC_NAME"
