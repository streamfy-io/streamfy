...
@test "Can create a mirror topic" {
    ...
    run timeout 15s "$STREAMFY_BIN" topic create "$TOPIC_NAME" --mirror-apply remotes.json
...
@test "Can add a new remote to the mirror topic" {
    run timeout 15s "$STREAMFY_BIN" topic add-mirror "$TOPIC_NAME" "$REMOTE_NAME_2"
...
@test "Can't add a non existent remote to the mirror topic" {
    run timeout 15s "$STREAMFY_BIN" topic add-mirror "$TOPIC_NAME" "nonexistent-remote"
...
@test "Can't add a remote to the mirror topic that doesn't exist" {
    run timeout 15s "$STREAMFY_BIN" topic add-mirror "nonexistent-topic" "$REMOTE_NAME"
...
@test "Can't add a remote to the mirror topic that is already assigned" {
    run timeout 15s "$STREAMFY_BIN" topic add-mirror "$TOPIC_NAME" "$REMOTE_NAME"
