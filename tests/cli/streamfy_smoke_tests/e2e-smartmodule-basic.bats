...
@test "smartmodule map" {
    ...
    run timeout 15s "$STREAMFY_BIN" smartmodule create $SMARTMODULE_NAME --wasm-file $SMARTMODULE_BUILD_DIR/streamfy_smartmodule_map.wasm
    ...
    TEST_MESSAGE="Banana"
    export TEST_MESSAGE
    run bash -c 'echo "$TEST_MESSAGE" | timeout 15s "$STREAMFY_BIN" produce "$TOPIC_NAME"'
    echo "cmd: $BATS_RUN_COMMAND" >&2
    assert_success

    EXPECTED_OUTPUT="BANANA"
    export EXPECTED_OUTPUT
    run timeout 15s "$STREAMFY_BIN" consume "$TOPIC_NAME" -B -d --smartmodule "$SMARTMODULE_NAME"
    echo "cmd: $BATS_RUN_COMMAND" >&2
    ...
    # Delete smartmodule
    run timeout 15s "$STREAMFY_BIN" smartmodule delete "$SMARTMODULE_NAME"
    echo "cmd: $BATS_RUN_COMMAND" >&2
    assert_success
}
...
@test "smartmodule filter" {
    ...
    export SMARTMODULE_NAME
    run timeout 15s "$STREAMFY_BIN" smartmodule create $SMARTMODULE_NAME --wasm-file $SMARTMODULE_BUILD_DIR/streamfy_smartmodule_filter.wasm
    echo "cmd: $BATS_RUN_COMMAND" >&2
    assert_success
    ...
    run bash -c 'echo "$NEGATIVE_TEST_MESSAGE" | timeout 15s "$STREAMFY_BIN" produce "$TOPIC_NAME"'
    echo "cmd: $BATS_RUN_COMMAND" >&2
    assert_success

    TEST_MESSAGE="$(random_string 10)aaa"
    export TEST_MESSAGE
    run bash -c 'echo "$TEST_MESSAGE" | timeout 15s "$STREAMFY_BIN" produce "$TOPIC_NAME" --compression gzip'
    echo "cmd: $BATS_RUN_COMMAND" >&2
    assert_success
...
@test "smartmodule filter w/ params" {
    ...
    export SMARTMODULE_NAME
    run timeout 15s "$STREAMFY_BIN" smartmodule create $SMARTMODULE_NAME --wasm-file $SMARTMODULE_BUILD_DIR/streamfy_smartmodule_filter_param.wasm
    assert_success
    ...
    run bash -c 'echo "$DEFAULT_PARAM_MESSAGE" | timeout 15s "$STREAMFY_BIN" produce "$TOPIC_NAME"'
    assert_success

    TEST_PARAM_MESSAGE="zzzzz"
    export TEST_PARAM_MESSAGE
    run bash -c 'echo "$TEST_PARAM_MESSAGE" | timeout 15s "$STREAMFY_BIN" produce "$TOPIC_NAME"'
    assert_success
    ...
    export EXPECTED_OUTPUT
    run timeout 15s "$STREAMFY_BIN" consume "$TOPIC_NAME" -B -d --smartmodule "$SMARTMODULE_NAME" --params key=z
    refute_line --partial "$NEGATIVE_TEST_MESSAGE"
...
@test "smartmodule filter-map" {
    ...
    export SMARTMODULE_NAME
    run timeout 15s "$STREAMFY_BIN" smartmodule create $SMARTMODULE_NAME --wasm-file $SMARTMODULE_BUILD_DIR/streamfy_smartmodule_filter_map.wasm
    assert_success
    ...
    TEST_MESSAGE="100"
    export TEST_MESSAGE
    run bash -c 'echo "$TEST_MESSAGE" | timeout 15s "$STREAMFY_BIN" produce "$TOPIC_NAME" --compression snappy'
    assert_success
...
@test "smartmodule array-map" {
    ...
    export SMARTMODULE_NAME
    run timeout 15s "$STREAMFY_BIN" smartmodule create $SMARTMODULE_NAME --wasm-file $SMARTMODULE_BUILD_DIR/streamfy_smartmodule_array_map_object.wasm
    ...
    run bash -c 'echo "$FULL_TEST_MESSAGE" | timeout 15s "$STREAMFY_BIN" produce "$TOPIC_NAME" --compression lz4'
    assert_success
...
@test "smartmodule aggregate" {
    ...
    export SMARTMODULE_NAME
    run timeout 15s "$STREAMFY_BIN" smartmodule create $SMARTMODULE_NAME --wasm-file $SMARTMODULE_BUILD_DIR/streamfy_smartmodule_aggregate.wasm
    echo "cmd: $BATS_RUN_COMMAND" >&2
    assert_output "smartmodule \"$SMARTMODULE_NAME\" has been created."
    ...
    run bash -c 'echo "$TEST_MESSAGE_1" | timeout 15s "$STREAMFY_BIN" produce "$TOPIC_NAME"'
    echo "cmd: $BATS_RUN_COMMAND" >&2
    assert_success

    TEST_MESSAGE_2="def"
    export TEST_MESSAGE_2
    run bash -c 'echo "$TEST_MESSAGE_2" | timeout 15s "$STREAMFY_BIN" produce "$TOPIC_NAME"'
    echo "cmd: $BATS_RUN_COMMAND" >&2
    assert_success
