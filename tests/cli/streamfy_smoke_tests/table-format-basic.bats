...
@test "Create table-format" {
    run timeout 15s "$STREAMFY_BIN" table-format create --config "$TABLE_FORMAT_CONFIG"
...
@test "List table-format" {
    run timeout 15s "$STREAMFY_BIN" table-format list
    assert_success
}

# Delete table-format
@test "Delete table-format" {
    run timeout 15s "$STREAMFY_BIN" table-format delete "$TABLE_FORMAT_NAME"
    assert_success
}
