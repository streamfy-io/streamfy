...
@test "Create smartmodule" {
    run timeout 15s "$STREAMFY_BIN" smartmodule create "$SMARTMODULE_NAME" --wasm-file "$(mktemp)"
...
@test "Describe smartmodule" {
    ...
    run timeout 15s "$STREAMFY_BIN" smartmodule describe "$SMARTMODULE_NAME"
    debug_msg "status: $status"
...
@test "List smartmodule" {
    run timeout 15s "$STREAMFY_BIN" smartmodule list
    debug_msg "status: $status"
    debug_msg "output: $output"
    assert_success
}
...
# https://github.com/streamfy/streamfy/issues/3634
#
# Download smartmodule from hub and test it without creating a new project
# @test "Test external smartmodule" {
#       run timeout 15s "$STREAMFY_BIN" hub sm download "streamfy/regex-filter@0.1.0"
#       assert_success
#       run timeout 15s "$STREAMFY_BIN" sm test \
#           -t '{"uses":"streamfy/regex-filter@0.1.0", "with": {"regex": "^f"}}' \
#           --text 'streamfy'
#       assert_output --partial streamfy
# }
