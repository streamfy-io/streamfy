...
# regression test issue https://github.com/streamfy/streamfy/issues/4161
@test "Produce message larger than batch size" {
    ...
    debug_msg small 25
    run bash -c 'timeout 65s "$STREAMFY_BIN" produce "$TOPIC_NAME" --batch-size 5000000 --max-request-size 100000000 --file $TOPIC_NAME-small.txt --raw --compression gzip'
    assert_success

    debug_msg med 50+1
    run bash -c 'timeout 65s "$STREAMFY_BIN" produce "$TOPIC_NAME" --batch-size 5000000 --max-request-size 100000000 --file $TOPIC_NAME-med.txt --raw --compression gzip'
    assert_success

    # should create a single batch for this record 
    debug_msg big 150
    run bash -c 'timeout 65s "$STREAMFY_BIN" produce "$TOPIC_NAME" --batch-size 5000000 --max-request-size 100000000 --file $TOPIC_NAME-big.txt --raw --compression gzip'
    assert_success
}
...
@test "Produce message larger than batch size with loop" {
	...
	debug_msg small 25
	run bash -c 'timeout 65s "$STREAMFY_BIN" produce "$TOPIC_NAME" --batch-size 5000000 --max-request-size 100000000 --file $TOPIC_NAME-loop.txt --raw --compression gzip'
	assert_success
    done
}
...
@test "Produce message larger then max request size" {
    ...
    run bash -c 'timeout 65s "$STREAMFY_BIN" produce "$TOPIC_NAME" --batch-size 5000000 --max-request-size 15000000 --file $TOPIC_NAME.txt --raw --compression gzip'
    assert_success
