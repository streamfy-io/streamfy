...
EOF
    ...
    run timeout 15s "$STREAMFY_BIN" sm create --wasm-file smartmodule/examples/target/wasm32-wasip1/release-lto/streamfy_smartmodule_filter.wasm "$DEDUP_FILTER_NAME"
    assert_success
}
...
@test "Create a topic with replica assignment" {
    ...
    SPU_1_ID="$($STREAMFY_BIN cluster spu list -O json | grep -v 'Current channel' | jq '.[0].spec.spuId')"
    SPU_2_ID="$($STREAMFY_BIN cluster spu list -O json | grep -v 'Current channel' | jq '.[1].spec.spuId')"
...
EOF
    ...
    run timeout 15s "$STREAMFY_BIN" topic create "$TOPIC_NAME_REPLICA" --replica-assignment "$REPLICA_CONFIG_PATH"
...
@test "Attempt to create topic with specified max partition size" {
    run timeout 15s "$STREAMFY_BIN" topic create "$(random_string)" --max-partition-size 10Gb --dry-run
    debug_msg "status: $status"
    debug_msg "output: ${lines[0]}"
    assert_success
}
...
@test "Attempt to create topic with specified segment size" {
    run timeout 15s "$STREAMFY_BIN" topic create "$(random_string)" --segment-size "2 Ki" --dry-run
    debug_msg "status: $status"
    debug_msg "output: ${lines[0]}"
    assert_success
}
...
@test "Attempt to create topic with too small max partition size" {
    run timeout 15s "$STREAMFY_BIN" topic create "$(random_string)" --max-partition-size "10" --dry-run
    debug_msg "status: $status"
    debug_msg "output: ${lines[0]}"
    assert_failure
...
@test "Attempt to create topic with max partition size smaller than segment size" {
    run timeout 15s "$STREAMFY_BIN" topic create "$(random_string)" --segment-size "3 Ki" --max-partition-size "2 Ki" --dry-run
    debug_msg "status: $status"
    debug_msg "output: ${lines[0]}"
    assert_failure
...
@test "Attempt to create topic with empty name" {
    run timeout 15s "$STREAMFY_BIN" topic create " " --dry-run
    debug_msg "status: $status"
    debug_msg "output: ${lines[0]}"
    assert_failure
...
@test "Attempt to create topic with name and config file" {
    run timeout 15s "$STREAMFY_BIN" topic create "$(random_string)" --config /tmp/config.yaml --dry-run
    debug_msg "status: $status"
    debug_msg "output: ${lines[0]}"
    assert_failure
...
@test "Attempt to create topic with partition size and config file" {
    run timeout 15s "$STREAMFY_BIN" topic create --max-partition-size "2 Ki"  --config /tmp/config.yaml --dry-run
    debug_msg "status: $status"
    debug_msg "output: ${lines[0]}"
    assert_failure
    assert_output --partial "error: the argument '--config <PATH>' cannot be used with"
...
@test "Create a topic from config file" {
    ...
    run timeout 15s "$STREAMFY_BIN" topic create --config "$TOPIC_CONFIG_PATH"
    debug_msg "status: $status"
    debug_msg "output: ${lines[0]}"
    assert_success
}
...
@test "Describe a topic with deduplication info" {
    if [ "$STREAMFY_CLI_RELEASE_CHANNEL" == "stable" ]; then
        skip "don't run on streamfy cli stable version"
    fi
    if [ "$STREAMFY_CLUSTER_RELEASE_CHANNEL" == "stable" ]; then
...
@test "Create a system topic" {
    ...
    run timeout 15s "$STREAMFY_BIN" topic create "$TOPIC_NAME_SYSTEM" --system
    assert_output --partial "topic "\"$TOPIC_NAME_SYSTEM"\" created"
    assert_success

    # Check if the topic is a system topic
    run bash -c 'timeout 15s "$STREAMFY_BIN" partition list --system | grep "$TOPIC_NAME_SYSTEM"'
