...
@test "Add 2 new partitions to the topic" {
    ...
    run timeout 15s "$STREAMFY_BIN" topic add-partition "$TOPIC_NAME" -c 2
...
@test "List partitions should have 3 partitions" {
    ...
    run timeout 15s "$STREAMFY_BIN" partition list

    #TODO: filter without grep when streamfy partition list has a filter option
...
@test "Add partitions to a topic that doesn't exist" {
    ...
    run timeout 15s "$STREAMFY_BIN" topic add-partition "not-exist-topic" -c 2
...
@test "Produce message to new partitions while connected" {
    ...
    spawn "$STREAMFY_BIN" produce "$TOPIC_NAME"
    expect "> "
    send "1\r"
    expect "Ok!"
    expect "> "
    ...
    expect "Ok!"
    expect "> "
    exec "$STREAMFY_BIN" topic add-partition "$TOPIC_NAME"
    sleep 2
    send "4\r"
    expect "Ok!"
    expect "> "
