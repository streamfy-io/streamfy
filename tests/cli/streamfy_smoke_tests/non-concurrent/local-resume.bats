...
run_list_spus() {
    ...
    # We must fail with `streamfy cluster` (hence -o pipefail) to propagate whether the command failed.
    run bash -o pipefail -c "\"$STREAMFY_BIN\" cluster spu list -O json | jq 'length'"
}

run_resume() {
    OUTPUT_FILE=${1:-/dev/null}
    # Since `streamfy run sc` runs sc as a child process without daemonizing it, all the FDs
    ...
    run_resume_cmd="nohup $STREAMFY_BIN cluster resume 0<&- &>${OUTPUT_FILE}"
...
@test "Resume cluster maintains SPU replica number" {
    # Reenable when streamfy cluster upgrade is ready
    skip

    run timeout 15s "$STREAMFY_BIN" cluster shutdown
    assert_success
...
@test "Can not start a running cluster" {
    ...
    run timeout 15s "$STREAMFY_BIN" cluster start
    assert_failure
}
