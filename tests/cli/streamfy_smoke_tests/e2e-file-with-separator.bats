...
@test "Produce file message with separator" {    
    run bash -c 'timeout 15s "$STREAMFY_BIN" produce --file "$MULTI_LINE_FILE_NAME" --key-separator "$SEPARATOR" "$TOPIC_NAME"'
    assert_success
}
...
@test "Consume file message with separator" {    
    run timeout 15s "$STREAMFY_BIN" consume "$TOPIC_NAME" -B -d -F '{{key}}={{value}}'
