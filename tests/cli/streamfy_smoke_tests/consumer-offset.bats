...
setup_file() {
    ...
    run timeout 15s "$STREAMFY_BIN" topic create "$TOPIC_NAME" --partitions 2

    echo 1:1 | "$STREAMFY_BIN" produce $TOPIC_NAME --key-separator ":"
    echo 2:2 | "$STREAMFY_BIN" produce $TOPIC_NAME --key-separator ":"
    echo 4:4 | "$STREAMFY_BIN" produce $TOPIC_NAME --key-separator ":"
    echo 8:8 | "$STREAMFY_BIN" produce $TOPIC_NAME --key-separator ":"
}
...
@test "Consumer Offsets topic exist" {
    ...
    while [ $SECONDS -lt $end_time ]; do
      SYSTEM_TOPIC_NAME="$($STREAMFY_BIN topic list --system -O json | jq '.[0].name' | tr -d '"')"
...
@test "Read one partition with consumer" {
    ...
    run timeout 15s "$STREAMFY_BIN" consume "$TOPIC_NAME" --consumer $CONSUMER_NAME -p 1 -B -d
    assert_success
    assert_line "4"
    assert_line "8"

    OFFSET=$("$STREAMFY_BIN" consumer list -O json | jq ".[] | select(.consumer_id == \"$CONSUMER_NAME\") | .offset")
    assert [ $OFFSET == "1" ]
...
@test "Read many partitions with consumer" {
    ...
    CONSUMER_NAME=$(random_string)
    run timeout 15s "$STREAMFY_BIN" consume "$TOPIC_NAME" --consumer $CONSUMER_NAME -B -d
    assert_success
    assert_line "1"
    assert_line "2"
    assert_line "4"
    ...
    OFFSET1=$("$STREAMFY_BIN" consumer list -O json | jq ".[] | select(.consumer_id == \"$CONSUMER_NAME\" and .partition == 0) | .offset")
    assert [ $OFFSET1 == "1" ]
    OFFSET2=$("$STREAMFY_BIN" consumer list -O json | jq ".[] | select(.consumer_id == \"$CONSUMER_NAME\" and .partition == 1) | .offset")
    assert [ $OFFSET2 == "1" ]
...
@test "Delete topic must delete consumers" {
    ...
    run timeout 15s "$STREAMFY_BIN" consumer list
...
@test "Delete topic must delete consumers and allow to create the same topic and consumer again" {
    ...
    run timeout 15s "$STREAMFY_BIN" topic create "$TOPIC_NAME" -p 2 -r 2
...
@test "End offset with auto as strategy should commit and flush the offset" {
    ...
    echo 3:3 | "$STREAMFY_BIN" produce $TOPIC_NAME --key-separator ":"
    echo 4:4 | "$STREAMFY_BIN" produce $TOPIC_NAME --key-separator ":"

    # consumer with end offset
    run timeout 15s "$STREAMFY_BIN" consume "$TOPIC_NAME" --consumer "$CONSUMER_NAME" -d
    assert_success
    assert_line "3"
    assert_line "4"
