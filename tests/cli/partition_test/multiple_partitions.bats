...
teardown_file() {
    ...
    "$STREAMFY_BIN" topic delete "$PRODUCE_CONSUME_MULTIPLE_PARTITIONS_TOPIC_NAME"
}
...
@test "Produces on topic for P/C Multiple Partitions" {
    run bash -c 'timeout 15s "$STREAMFY_BIN" produce --file "$MULTI_LINE_FILE_NAME" "$PRODUCE_CONSUME_MULTIPLE_PARTITIONS_TOPIC_NAME"'
    assert_success
}
