...
setup_file() {
    ...
    if [[ -z "$SKIP_SETUP" ]];
    then
        setup_streamfy_cluster "$CLUSTER_VERSION"
        setup_streamfy_cli "$CLI_VERSION"
    else
...
teardown_file() {
    ...
    then
        ...
        "$STREAMFY_CLIENT_BIN" cluster delete --force
    else
...
@test "Create a topic: $TOPIC_NAME" {
    ...
    run timeout 15s "$STREAMFY_CLIENT_BIN" topic create "$TOPIC_NAME"
    assert_success
}

# Produce message
@test "Produce message" {
    run bash -c 'echo "$MESSAGE" | timeout 15s "$STREAMFY_CLIENT_BIN" produce "$TOPIC_NAME"'
...
# List topics
@test "List topics" {
    run timeout 15s "$STREAMFY_CLIENT_BIN" topic list
...
@test "List partitions" {
    run timeout 15s "$STREAMFY_CLIENT_BIN" partition list
...
@test "Consume message" {
    run timeout 15s "$STREAMFY_CLIENT_BIN" consume "$TOPIC_NAME" -B -d
