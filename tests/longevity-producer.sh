...
function setup() {
    ...
    # Start a cluster
    $STREAMFY_BIN cluster start --k8 --image-version latest

    # Create a topic to delete at the end
    #$STREAMFY_BIN topic create $NEW_TOPIC_NAME || true

    # Create a topic, if it doesn't exist, that should stick around for future tests
    $STREAMFY_BIN topic create $EXISTING_TOPIC_NAME || true
...
function test_produce() {
    ...
    do
        ...
        echo ${PRODUCER_PAYLOAD} | $STREAMFY_BIN produce ${TOPIC_NAME} 
