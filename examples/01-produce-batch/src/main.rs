//! A minimal example showing how to produce batches of records with Streamfy
//!
//! Before running this example, make sure you have created a topic
//! named `batch` with the following command:
//!
//! ```text
//! $ streamfy topic create batch
//! ```
//!
...
async fn produce_batch() -> anyhow::Result<()> {
    let producer = streamfy::producer("batch").await?;
