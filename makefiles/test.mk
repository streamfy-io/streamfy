TEST_BIN?=$(if $(TARGET),./target/$(TARGET)/$(BUILD_PROFILE)/streamfy-test,./target/$(BUILD_PROFILE)/streamfy-test)
CLIENT_LOG?=warn
SERVER_LOG?=info
DEFAULT_SPU?=2
REPL?=2
...
SPU_DELAY?=5
SC_AUTH_CONFIG?=./crates/streamfy-sc/test-data/auth_config
EXTRA_ARG?=
...
test-permission-k8:	SC_PORT=$(shell kubectl ${KUBECTL_ARG_NAMESPACE} get svc streamfy-sc-public -o json | jq '.spec.ports[0].nodePort' )
...
install-test-k8-port-forwarding:
	$(STREAMFY_BIN) cluster start --k8 --develop --use-k8-port-forwarding
...
# When running in development, might need to run `cargo clean` to ensure correct streamfy binary is used
validate-release-stable:
	./tests/streamfy-validate-release.sh $(VERSION) $(GIT_COMMIT)
...
cli-streamfy-smoke:
	bats -x $(shell ls -1 ./tests/cli/streamfy_smoke_tests/*.bats | sort -R)
...
cli-streamfy-mirroring-smoke:
	bats $(shell ls -1 ./tests/cli/mirroring_smoke_tests/*.bats | sort -R)

cli-streamfy-mirroring-smoke-e2e:
...
cli-svm-smoke:
	bats $(shell ls -1 ./tests/cli/svm_smoke_tests/*.bats | sort -R)

cli-basic-test:
	bats ./tests/cli/streamfy_smoke_tests/e2e-basic.bats

cli-smartmodule-all-test: sm-target
	bats ./tests/cli/streamfy_smoke_tests/e2e-smartmodule-basic.bats

cli-smartmodule-aggregate-test: sm-target
	bats -f aggregate ./tests/cli/streamfy_smoke_tests/e2e-smartmodule-basic.bats

cli-smartmodule-basic-test: sm-target
	bats ./tests/cli/streamfy_smoke_tests/smartmodule-basic.bats

cli-producer-smartmodule-test: sm-target
	bats ./tests/cli/streamfy_smoke_tests/producer-smartmodule.bats
...
cli-svm-basic-test:
	SVM_BIN=$(shell readlink -f $(SVM_BIN)) bats   ./tests/cli/svm_smoke_tests/svm_basic.bats
...
test-rbac:
	AUTH_POLICY=$(POLICY_FILE) X509_AUTH_SCOPES=$(SCOPE) make smoke-test-tls DEFAULT_LOG=streamfy=debug
...
ifeq (${CI},true)
streamfy-test-self-check:
else
streamfy-test-self-check: build-test
endif
